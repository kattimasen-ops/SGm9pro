...
def patch_aim_assist():
    """
    v14.7: Target-Scan laeuft NACH Maus/Joystick-Input.
    ...
    helper_code = r"""
/* ============================================================
 * [PATCHED v14.7] Handheld Aim Assist (Client-Seite)
 * ============================================================ */
#include <math.h>
...
static qboolean HHA_HasLineOfSight( int entNum, const vec3_t fromFeet, const vec3_t toFeet ) {
    ...
    /* v14.7: MASK_PLAYERSOLID statt MASK_SHOT.
     * Enthaelt CONTENTS_PLAYERCLIP -> unsichtbare Barrieren
     * blockieren die Sichtlinie korrekt. */
...
static int HHA_FindTarget( const hha_config_t *cfg, vec3_t bestDir, float *bestAngleOut ) {
    ...
    *bestAngleOut = 999.0f;
    ...
    for ( i = 0; i < cl.snap.numEntities && ncands < HHA_MAX_CANDIDATES; i++ ) {
        ...
        /* v14.7: EF_DEAD UND EF_NODRAW ausschliessen */
        if ( ent->eFlags & (EF_DEAD | EF_NODRAW) ) continue;

        HHA_GetPredictedPos( ent, cfg, predFeet );

        /* v14.7: Position auf Map-Grenzen pruefen.
         * Ungueltige trDelta-Werte koennen Positionen weit
         * ausserhalb der Map erzeugen. */
...
"""
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"[PATCHED] Aim assist v14.7 in {target_file}")
    return True
...
def write_aim_menu(menu_dir):
    """
    v14.7: Preset-Auswahl als ITEM_TYPE_MULTI (Dropdown).
    Grund: UI-VM kennt weder 'cvar' noch 'setcvar' als Action.
    Der Client wendet Preset-Werte automatisch an.
    """
...
"""
    with open(os.path.join(menu_dir, "settings_aimassist.menu"),
        ...
        f.write(aim_menu)
    print("[PATCHED] settings_aimassist.menu (v14.7 Dropdown)")
...
def main():
    ...
    applied = 0
    if is_smokinguns:
        print("==> Patching SmokinGuns (v14.7)")
        diagnostic_dump()
