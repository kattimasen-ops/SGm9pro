...
v13.2 - Performance + AimBot + In-Game-Menue:
  - Config-Cache: HHA_ReadConfig nur 1x pro Frame statt 3x
  - LOS-Budget von 4 auf 1 gesenkt (CM_BoxTrace ist teuer auf RK3326)
  - Cvar_Set fuer Ziel-Distanz nur bei tatsaechlicher Aenderung
  - NEU: cg_handheldAimAssistHardLock (echter AimBot, instant Snap)
  - Cone-Cap auf 89 Grad wenn HardLock aktiv
  - ingame_options.menu wird ins Release-Verzeichnis kopiert
  - Respawn-Reset mit 4 unabhaengigen Triggern
  - Aimbot-Preset mit maximaler Effektivitaet
"""
...
def patch_client_cvar():
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write(content)
    print("[PATCHED] cl_main.c: 23 cvars registered")
    return True
 ...
 *   - Config wird 1x pro Frame gecached (statt 3x HHA_ReadConfig)
 *   - LOS-Budget auf 1 pro Frame reduziert
 *   - Cvar_Set nur bei tatsaechlicher Wert-Aenderung
 *
 * Neue Features:
 ...
 * Refresht Config-Cache und prueft Respawn.
 * ============================================================ */
...
static void CL_HandheldAimMagnetism( usercmd_t *cmd ) {
    ...
    /* Hard-Lock AimBot: instant Snap auf Ziel */
    if ( cfg->hardLock ) {
