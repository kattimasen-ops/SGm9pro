...
v15.0 - Client-seitiger Gegner-Radar (Snapshot + Sound).
...
def patch_client_cvar():
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write(content)
    print("[PATCHED] cl_main.c: 38 cvars registered")
    return True
    ...
    if '"cg_handheldAutoSwitch"' not in content:
        ...
        if anchor in content:
            entries = (
                ...
                '\n\t{ &ui_handheldRadarRadius, "cg_handheldRadarRadius", "100", CVAR_ARCHIVE },'
            ...
            print("[PATCHED] ui_main.c: 31 cvarTable entries")
...
def patch_cg_crosshair_min_size():
    ...
    if n == 0:
        return False
    print("[PATCHED] Crosshair Mindestgroesse 25 in cg_draw.c (CG_DrawCrosshair)")
...
'''
    ...
    if draw2d_pat.search(content):
        content = draw2d_pat.sub(
            r'\1\n\n\t/* HHA_HITMARKER_V4 */\n\tCG_HandheldDrawOverlays();',
            content, count=1)
        print("[PATCHED] Hitmarker + Damage-Indicator v14.9 (groesser)")
...
def patch_cg_hha_radar():
    ...
    if "HHA_RADAR_V1" in content:
        print("[SKIP] Radar v15.0 bereits gepatcht")
        return True

    radar_code = r'''

/* ============================================================
 * [PATCHED v15.0] Client-seitiger Gegner-Indikator
 * HHA_RADAR_V1
 * ============================================================ */
#include <math.h>

#define HHA_RADAR_MAX_ENTITIES       64
#define HHA_RADAR_SOUND_TIMEOUT      3000
#define HHA_RADAR_LOS_BUDGET         8
#define HHA_RADAR_MIN_DIST           64.0f
...
static hha_radar_entry_t hha_radarCache[HHA_RADAR_MAX_ENTITIES];
static int hha_radarNumEntries = 0;
static int hha_radarLosBudget = 0;
...
static float HHA_Radar_SoundStrength( int entNum ) {
    int dt;
    float fade;
    if ( entNum < 0 || entNum >= MAX_GENTITIES ) return 0.0f;
    dt = cg.time - hha_sndLastTime[entNum];
    if ( dt < 0 || dt > HHA_RADAR_SOUND_TIMEOUT ) return 0.0f;
    fade = 1.0f - ( (float)dt / HHA_RADAR_SOUND_TIMEOUT );
    return fade * ( (float)hha_sndLevel[entNum] / (float)HHA_SND_VLOUD );
}

static float HHA_Radar_CvarFloat( const char *name ) {
    char buf[16];
    buf[0] = 0;
    trap_Cvar_VariableStringBuffer( name, buf, sizeof(buf) );
    return (float)atof( buf );
...
static qboolean HHA_Radar_IsEnemy( entityState_t *ent ) {
...
void CG_HHA_Radar_Update( void ) {
    ...
    hha_radarNumEntries = 0;
    hha_radarLosBudget   = HHA_RADAR_LOS_BUDGET;
    ...
    {
        char buf[16];
        trap_Cvar_VariableStringBuffer("cg_handheldRadar", buf, sizeof(buf));
        if ( atof(buf) == 0.0f ) return;
    }

    maxDist = HHA_Radar_CvarFloat("cg_handheldRadarRange");
    if ( maxDist < 500.0f )   maxDist = 500.0f;
    if ( maxDist > 8000.0f )  maxDist = 8000.0f;

    AngleVectors( cg.refdefViewAngles, forward, right, NULL );

    for ( i = 0; i < cg.snap->numEntities && hha_radarNumEntries < HHA_RADAR_MAX_ENTITIES; i++ ) {
        entityState_t *ent  = &cg.snap->entities[i];
        centity_t     *cent = &cg_entities[ent->number];
        hha_radar_entry_t *e;
        float dotF, dotR, distSq;

        if ( !HHA_Radar_IsEnemy( ent ) ) continue;

        VectorSubtract( cent->lerpOrigin, cg.refdef.vieworg, delta );
        distSq = delta[0]*delta[0] + delta[1]*delta[1] + delta[2]*delta[2];
        if ( distSq < HHA_RADAR_MIN_DIST * HHA_RADAR_MIN_DIST ) continue;
        if ( distSq > maxDist * maxDist ) continue;

        e = &hha_radarCache[hha_radarNumEntries];
        e->entNum     = ent->number;
        e->distance   = sqrtf( distSq );
        e->isLooping  = hha_sndLooping[ent->number];
        e->soundStrength = HHA_Radar_SoundStrength( ent->number );

        VectorCopy( cent->lerpOrigin, e->worldPos );
        VectorCopy( ent->pos.trDelta, e->velocity );

        VectorNormalize( delta );
        dotF = DotProduct( forward, delta );
        dotR = DotProduct( right,   delta );
        e->screenAngle = atan2f( dotR, dotF ) * ( 180.0f / M_PI );

        e->hasLoS = qfalse;
        if ( hha_radarLosBudget > 0 ) {
            trace_t tr;
            vec3_t eye, chest;
            hha_radarLosBudget--;
            VectorCopy( cg.refdef.vieworg, eye );
            VectorCopy( e->worldPos, chest );
            chest[2] += 32.0f;
            CG_Trace( &tr, eye, NULL, NULL, chest,
                      cg.snap->ps.clientNum, MASK_SHOT );
            e->hasLoS = ( tr.fraction >= 0.999f );
        }
...
void CG_HHA_Radar_Draw( void ) {
    int   i;
    float cx, cy, ringR;
    float baseSize = 6.0f;
    float ringColor[4] = { 0.3f, 0.3f, 0.3f, 0.35f };

    if ( !cg.snap ) return;
    if ( cg.snap->ps.stats[STAT_HEALTH] <= 0 ) return;
    if ( cg.renderingThirdPerson ) return;

    if ( HHA_Radar_CvarFloat("cg_handheldRadar") == 0.0f ) return;

    cx = 320.0f;
    cy = 240.0f;
    ringR = HHA_Radar_CvarFloat("cg_handheldRadarRadius");
    if ( ringR < 40.0f )  ringR = 40.0f;
    if ( ringR > 200.0f ) ringR = 200.0f;

    trap_R_SetColor( ringColor );
    for ( i = 0; i < 64; i++ ) {
        float a = (float)i * ( 360.0f / 64.0f ) * ( M_PI / 180.0f );
        float px = cx + sinf( a ) * ringR - 1.0f;
        float py = cy - cosf( a ) * ringR - 1.0f;
        CG_DrawPic( px, py, 2.0f, 2.0f, cgs.media.whiteShader );
    }
    trap_R_SetColor( NULL );

    for ( i = 0; i < hha_radarNumEntries; i++ ) {
        hha_radar_entry_t *e = &hha_radarCache[i];
        float a, px, py, size;
        float color[4];

        a  = e->screenAngle * ( M_PI / 180.0f );
        px = cx + sinf( a ) * ringR;
        py = cy - cosf( a ) * ringR;

        size = baseSize + e->soundStrength * 8.0f;
        size += ( 1.0f - ( e->distance / 3000.0f ) ) * 3.0f;
        if ( size < 4.0f )  size = 4.0f;
        if ( size > 16.0f ) size = 16.0f;

        if ( e->hasLoS ) {
            color[0] = 1.0f; color[1] = 0.15f; color[2] = 0.05f;
        } else if ( e->soundStrength > 0.4f ) {
            color[0] = 1.0f; color[1] = 0.65f; color[2] = 0.1f;
        } else if ( e->soundStrength > 0.05f || e->isLooping ) {
            color[0] = 0.8f; color[1] = 0.85f; color[2] = 0.15f;
        } else {
            color[0] = 0.4f; color[1] = 0.4f; color[2] = 0.4f;
        }
        color[3] = 0.35f + e->soundStrength * 0.65f;
        if ( color[3] > 1.0f ) color[3] = 1.0f;

        trap_R_SetColor( color );
        CG_DrawPic( px - size * 0.5f, py - size * 0.5f,
                    size, size, cgs.media.whiteShader );

        if ( e->soundStrength > 0.7f ) {
            float pulse = 1.0f + 0.4f * sinf( (float)cg.time * 0.02f );
            float ps = size * pulse;
            color[3] *= 0.5f;
            trap_R_SetColor( color );
            CG_DrawPic( px - ps * 0.5f, py - ps * 0.5f,
                        ps, ps, cgs.media.whiteShader );
        }
...
'''
    ...
    if hook_pat.search(content):
        content = hook_pat.sub(
            r'\1\n\n\t/* HHA_RADAR_V1: Update + Draw */\n'
            r'\tCG_HHA_Radar_Update();\n'
            r'\tCG_HHA_Radar_Draw();',
            content, count=1
        )
        print("[PATCHED] Radar v15.0 in cg_draw.c")
    else:
        print("[WARN] cg_draw.c: Radar-Hook nicht eingebaut")
...
def patch_cg_ents_sound_hook():
    ...
    decl_block = (
        "\n/* HHA_ENTS_SOUND_TRACK: Sound-Tracking fuer Radar */\n"
    ...
    hook_pat = re.compile(
        r'(\tCG_SetEntitySoundPosition\(\s*cent\s*\)\s*;)'
    )
    ...
    content = hook_pat.sub(
        r'\1\n\n'
        r'\t/* HHA_ENTS_SOUND_TRACK: Loop-Sound tracken */\n'
    ...
    print("[PATCHED] cg_ents.c: Sound-Tracking in CG_EntityEffects")
...
'''
    ...
    content = hook_pat.sub(
        r'\1\n\n'
        r'\t/* HHA_EVENT_SOUND_TRACK: Sound-Event fuer Radar */\n'
        r'\tHHA_TrackEntityEvent( cent, event );',
        content, count=1
    )
    print("[PATCHED] cg_event.c: Sound-Tracking in CG_EntityEvent")
...
def patch_cg_servercmds_rename_filter():
    ...
    new_block = (
        '\tif ( !strcmp( cmd, "cp" ) ) {\n'
        '\t\t/* HHA_RENAME_CP_FILTER: Unterdrueckt Center-Prints,\n'
        '\t\t * die nur den eigenen Spielernamen enthalten. */\n'
        '\t\t{\n'
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print("[PATCHED] cg_servercmds.c: Rename-Centerprint-Filter v14.9")
    return True


def patch_hud_killfeed_filter():
    """v15.1: Filtert 'X killed Y with Z'-Nachrichten aus dem Chat."""
    target_file = os.path.join("code", "cgame", "cg_servercmds.c")
    if not os.path.exists(target_file):
        for root, _dirs, files in os.walk("code"):
            if "cg_servercmds.c" in files:
    ...
    helper = r'''
/* ============================================================
 * [PATCHED v15.1] Kill-Chat-Filter
 * HHA_KILLFEED_FILTER
 * ============================================================ */
static qboolean HHA_IsKillMessage( const char *text ) {
    ...
    if ( strstr( t, " killed " ) != NULL &&
         strstr( t, " with " )   != NULL ) {
        return qtrue;
    }
    if ( strstr( t, "teamkilled" ) != NULL ) {
        return qtrue;
    }
    return qfalse;
}
...
'''
    ...
    new_chat = (
        ...
        '\t\t\tQ_strncpyz( text, CG_Argv(1), MAX_SAY_TEXT );\n'
        '\t\t\tCG_RemoveChatEscapeChar( text );\n'
        '\t\t\t/* HHA_KILLFEED_FILTER: Kill-Chat unterdruecken */\n'
        '\t\t\tif ( Cvar_VariableIntegerValue("cg_handheldHideKillChat") &&\n'
        '\t\t\t     HHA_IsKillMessage( text ) ) {\n'
        '\t\t\t\treturn;\n'
        '\t\t\t}\n'
        '\t\t\tif ( !cg_chatMode.integer ) {\n'
        '\t\t\t\tCG_Printf( "%s\\n", text );\n'
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print("[PATCHED] cg_servercmds.c: Kill-Chat-Filter v15.1")
    return True


def patch_hud_obituary_filter():
    """v15.1: Filtert CG_Printf in CG_Obituary (Weapon-Kill)."""
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_event.c" in files:
            target_file = os.path.join(root, "cg_event.c")
    ...
    new_block = (
        '\tif(weapon){\n'
        '\t\t/* HHA_OBITUARY_FILTER: Chat-Ausgabe unterdrueckbar */\n'
        '\t\tif ( !Cvar_VariableIntegerValue("cg_handheldHideKillChat") ) {\n'
        '\t\t\tif ( ent->eFlags & EF_SAME_TEAM )\n'
        '\t\t\t\tCG_Printf("%s ^1teamkilled^7 %s with %s.\\n", attackerName, targetName, bg_weaponlist[weapon].name );\n'
        '\t\t\telse if ( cg_killmsg.integer == 1 )\n'
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print("[PATCHED] cg_event.c: Obituary-Chat-Filter v15.1")
    return True


def patch_hud_hitmessage_filter():
    """v15.2: Filtert Treffer-Meldungen."""
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_event.c" in files:
            target_file = os.path.join(root, "cg_event.c")
    ...
    guard = (
        ...
        "\t/* HHA_HITMSG_FILTER: Treffer-Meldungen unterdrueckbar */\n"
        "\tif ( Cvar_VariableIntegerValue(\"cg_handheldHideKillChat\") ) {\n"
        "\t\treturn;\n"
        "\t}\n"
    )
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print("[PATCHED] cg_event.c: Hitmessage-Filter v15.2")
    return True


def patch_hud_killfeed_move_left():
    """v15.1: Verschiebt den grafischen Kill-Feed nach oben links."""
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_draw.c" in files:
            target_file = os.path.join(root, "cg_draw.c")
    ...
    new_pos = (
        '\t\t\t/* HHA_KILLFEED_LEFT: nach oben links statt rechts */\n'
        '\t\t\tx = 4;'
    )
    ...
    # Aus CG_DrawUpperRight entfernen
    ...
    if upper_right_call in content:
        ...
        print("[PATCHED] DeathMessages aus CG_DrawUpperRight entfernt")

    # In CG_Draw2D einfuegen (verankert an Radar-Update)
    hook_anchor = 'CG_HHA_Radar_Update();'
    if hook_anchor in content:
        content = content.replace(
            hook_anchor,
            '/* HHA_KILLFEED_LEFT: DeathMessages links oben */\n'
            '\t{\n'
            '\t\tfloat kfY = 24.0f;\n'
...
def patch_hud_compass():
    ...
    if "HHA_COMPASS_V1" in content:
        print("[SKIP] Compass v15.1 bereits gepatcht")
        return True

    compass_code = r'''

/* ============================================================
 * [PATCHED v15.1] Kompass oben Mitte
 * HHA_COMPASS_V1
 * ============================================================ */
#include <math.h>

#define HHA_COMPASS_STRIP_W   320.0f
#define HHA_COMPASS_STRIP_H    16.0f
...
void CG_HandheldDrawCompass( void ) {
    ...
    float colBg[4]      = { 0.00f, 0.00f, 0.00f, 0.45f };
    float colBorder[4]  = { 0.60f, 0.60f, 0.60f, 0.50f };
    ...
    for ( i = 0; i < 8; i++ ) {
        ...
        if ( diff < -HHA_COMPASS_VIEW_DEG || diff > HHA_COMPASS_VIEW_DEG ) continue;

        xpos = cx + ( diff / HHA_COMPASS_VIEW_DEG ) * halfW;

        col = ( i == 0 ) ? colActive : colNormal;
        col[3] *= 1.0f - ( fabsf( diff ) / HHA_COMPASS_VIEW_DEG ) * 0.4f;

        labelW = CG_Text_Width( hha_compass_names[i], 0.28f, 0 );
        CG_Text_Paint( xpos - labelW * 0.5f, cy + 13.0f, 0.28f, col,
                       hha_compass_names[i], 0, 0, ITEM_TEXTSTYLE_SHADOWED );

        trap_R_SetColor( col );
        CG_DrawPic( xpos - 0.5f, cy + HHA_COMPASS_STRIP_H - 4.0f, 1.0f, 3.0f,
                    cgs.media.whiteShader );
        trap_R_SetColor( NULL );
    }
}
...
'''
    ...
    if hook_anchor in content:
        ...
        print("[PATCHED] Compass v15.1 in cg_draw.c")
...
def patch_hud_low_health_vignette():
    """v15.1: Roter Rand bei niedriger Gesundheit."""
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_draw.c" in files:
            target_file = os.path.join(root, "cg_draw.c")
...
/* ============================================================
 * [PATCHED v15.1] Low-Health-Vignette
 * HHA_LOWHEALTH_V1
 * ============================================================ */
void CG_HandheldDrawLowHealthVignette( void ) {
    ...
    pulse = 1.0f;
    if ( hp < 15 ) {
        pulse = 0.6f + 0.4f * sinf( (float)cg.time * 0.008f );
    }
...
"""
    with open(os.path.join(menu_dir, "settings_aimassist.menu"),
        ...
        f.write(aim_menu)
    print("[PATCHED] settings_aimassist.menu (v14.8 Dropdown)")
...
def main():
    ...
    applied = 0
    if is_smokinguns:
        print("==> Patching SmokinGuns (v15.2)")
        diagnostic_dump()
