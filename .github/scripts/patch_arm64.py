...
v13.3.1 - Fix: Name-Rotator Helper auf File-Scope (nicht in CL_Frame)
  ALLE v13.1/v13.2/v13.3-Funktionen sind weiterhin enthalten (nichts entfernt).
  - FIX: patch_cl_main_namerotator() fuegt Helper-Funktionen VOR CL_Frame
    ein (vorher 'invalid storage class for function' Fehler)
  - v13.3: Name Color Rotator (cg_handheldNameRotate/Interval/Scheme)
  - v13.3: Chat-Prefill (cg_handheldChatPrefix) fuer T und Y
  - v13.2: Config-Cache, LOS-Budget 1, TargetDist-Setzen nur bei Aenderung
  - v13.2: cg_handheldAimAssistHardLock (instant Snap AimBot)
  - v13.2: preset_aimbot.cfg mit maximaler Effektivitaet
...
def patch_client_cvar():
    ...
    lines = [
        "\t/* [PATCHED v13.3.1] Handheld Aim Assist + Name Rotator + Chat Prefill */",
...
def patch_aim_assist():
    ...
    helper_code = r"""
/* ============================================================
 * [PATCHED v13.3.1] Handheld Aim Assist
 *
 * Performance (v13.2):
 *   - Config 1x pro Frame gecached (HHA_RefreshConfig)
 *   - LOS-Budget auf 1 pro Frame reduziert
 *   - Cvar_Set fuer Ziel-Distanz nur bei Aenderung
 *
 * Features:
 *   - cg_handheldAimAssistHardLock: instant Snap (AimBot)
 *   - Respawn-Reset mit 4 unabhaengigen Triggern
 *   - Alle v13.1-Features erhalten (Sticky, LOS-Cache,
 *     Unlagged-Sync, Interpolate, Lead, Snap, Falloff,
 *     Friction, Zoom)
 * ============================================================ */
#include <math.h>
...
"""
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"[PATCHED] Aim assist v13.3.1 in {target_file}")
    return True


def patch_cl_main_namerotator():
    """[v13.3.1] Rotiert nur die Farbcodes im name-Cvar, Text bleibt gleich.

    FIX gegenueber v13.3: Helper-Funktionen werden VOR CL_Frame eingefuegt
    (File-Scope), nicht in CL_Frame hinein. Sonst Fehler:
    'invalid storage class for function'.

    Der Aufruf HHA_NameRotator() erfolgt innerhalb CL_Frame direkt nach
    CL_CheckUserinfo().
    """
    ...
    helper_code = r"""
/* ============================================================
 * [PATCHED v13.3.1] Name Color Rotator
 * Rotiert nur die Farbcodes im name-Cvar. Der Text bleibt
 * unveraendert. Basis ist der gestrippte Name aus dem .sh,
 * z. B. ->St0len-One<- (mit Null statt o in "Stolen").
 * Aktiv nur wenn cg_handheldNameRotate != 0.
 *
 * WICHTIG: Diese Funktionen werden auf FILE-SCOPE definiert
 * (vor CL_Frame), nicht innerhalb einer anderen Funktion.
 * ============================================================ */
...
static void HHA_NameRotator(void) {
    ...
    /* Wenn der Name extern geaendert wurde (z.B. durch /name cmd),
       Basis-Text neu extrahieren. */
...
"""
    ...
    # Helper-Funktionen VOR CL_Frame einfuegen (File Scope!)
    ...
    # Aufruf INNERHALB CL_Frame, direkt nach CL_CheckUserinfo()
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print("[PATCHED] Name rotator in cl_main.c (v13.3.1, file-scope)")
    return True


def patch_cl_console_chatprefill():
    """[v13.3] Prefill fuer Chat-Eingabe bei T (say) und Y (say_team).

    Wenn cg_handheldChatPrefix gesetzt ist, wird das Chat-Feld beim
    Oeffnen automatisch mit diesem Text vorbelegt.
    """
...
def patch_cg_view():
    ...
    zoom_helper = r"""
/* ============================================================
 * [PATCHED v13.3.1] Dynamic Zoom + Auto-Switch
 * ============================================================ */
...
def patch_cg_hitmarker():
    ...
    hitmarker_code = r"""
/* ============================================================
 * [PATCHED v13.3.1] Hitmarker + Damage-Indicator
 * ============================================================ */
...
def patch_cg_crosshair_player():
    """Crosshair wird orange bei Ziel auf gegnerisches Playermodel."""
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_draw.c" in files:
            target_file = os.path.join(root, "cg_draw.c")
...
def patch_ingame_options_menu():
    """Fuegt einen Button zum Aim-Assist-Menue in ingame_options.menu ein
    UND kopiert die gepatchte Datei ins Release-Verzeichnis.
    """
    target_file = None
...
def main():
    ...
    applied = 0
    if is_smokinguns:
        print("==> Patching SmokinGuns (v13.3.1)")
        diagnostic_dump()
