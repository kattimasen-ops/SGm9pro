...
v13.7 - Server Native VM (.so) Support + Pure-Checksummen.
"""
...
def patch_cl_main_namerotator():
    ...
    helper_code = r"""
/* ============================================================
 * [PATCHED v13.5] Name Color Rotator (File-Scope)
 * ============================================================ */
...
"""
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print("[PATCHED] Name rotator in cl_main.c (v13.5)")
    return True
...
"""
    with open(os.path.join(menu_dir, "settings_aimassist.menu"),
        ...
        f.write(aim_menu)
    print("[PATCHED] Menu: settings_aimassist.menu (v13.5 setcvar-fix)")
...
# ============================================================
# [PATCHED v13.6] Native Shared Library (.so) VM Support (Client)
#
# Lädt native .so-Bibliotheken (smokinguns.so, cgame.so, ui.so)
# direkt aus dem BASEGAME-Ordner und bevorzugt sie gegenüber
# QVM-Dateien. Vorteile auf RK3326/Cortex-A35:
#   * Kein QVM-Interpreter-Overhead (direkter CPU-Code)
#   * Volle NEON/VFP-Nutzung durch aarch64-linux-gnu-gcc
#   * Kein 32-bit QVM-Speicherlimit
#   * Lauffähig ohne QVM-Interpreter im Client
# ============================================================
def patch_fs_native_vm():
    ...
    for root, _dirs, files in os.walk("code"):
        if "files.c" in files and "qcommon" in root:
            ...
            break
    if not files_c:
        print("[WARN] files.c nicht gefunden – Native-VM-Patch übersprungen")
        return touched
...
'''
    ...
    anchor = re.search(r'(qboolean\s+FS_FindVM\s*\()', content)
    if anchor:
        content = content[:anchor.start()] + helper + "\n" + content[anchor.start():]
    else:
        content = content + "\n" + helper + "\n"
...
# ============================================================
# [PATCHED v13.7] Server-Side Native Library VM Support
#
# Erweitert FS um native .so-Unterstützung (Server-Kontext):
#   * FS_FindVM()                    - Suche native .so > QVM
#   * FS_NativeFindFile()            - Datei-Lookup BASEGAME/*.so
#   * FS_ReferencedPakPureChecksums  - um native Summe erweitert
#   * FS_NativePakChecksumToString() - Server-Erwartung formatieren
#
# Wirkung: Server lädt qagame/smokinguns.so bevorzugt. Clients
# mit identischer Pure-Summe (Server-Expectation) werden
# akzeptiert, abweichende werden als "impure" abgelehnt.
# ============================================================
def patch_server_native_vm():
    print("==> Patching Server Native VM Support (v13.7)")
    touched = False
    ...
    if qcommon_h:
        ...
        if "VMI_NATIVE" not in qh:
            ...
            with open(qcommon_h, "w", encoding="utf-8") as f:
                f.write(qh)
            print("[PATCHED] qcommon.h: VMI_NATIVE added")
            touched = True
    ...
    if "FS_FindVM" in content:
        print("[SKIP] FS_FindVM bereits vorhanden")
        return touched

    helper = r'''
/* ============================================================
 * [PATCHED v13.7] Server-Side Native Library Support
 *
 * Dateiformat:  BASEGAME/basename.so   (ELF/AArch64)
 * Fallback:     BASEGAME/basename.dll  (PE/Windows)
 * QVM-Fallback: BASEGAME/basename.qvm  (Bytecode)
 * Puffer:       4096 Bytes für Pfade
 *
 * Pure-Checksummen: 8 Hex-Zeichen (CRC32), Format
 *   "@XXXXXXXX @YYYYYYYY ..."  (Server-Erwartung)
 * ============================================================ */

#define FS_NATIVE_MAX_PATH       4096
#define FS_NATIVE_CHECKSUM_HEX   9   /* @ + 8 Hex + NUL */
                            ...
                            char *outPath, int outSize )
{
    ...
    baseDir = fs_basegame ? fs_basegame->string : BASEGAME;

    Com_sprintf( testPath, sizeof( testPath ), "%s/%s.so", baseDir, basename );
    len = FS_FOpenFileRead( testPath, &f, qfalse );
    if ( len >= 0 ) {
        if ( f ) FS_FCloseFile( f );
        Q_strncpyz( outPath, testPath, outSize );
    ...
    Com_sprintf( testPath, sizeof( testPath ), "%s/%s.dll", baseDir, basename );
    len = FS_FOpenFileRead( testPath, &f, qfalse );
    if ( len >= 0 ) {
        if ( f ) FS_FCloseFile( f );
        Q_strncpyz( outPath, testPath, outSize );
...
qboolean FS_FindVM( const char *basename, char *outPath,
                    int outSize, vmType_t *vmType )
{
    char         qvmPath[FS_NATIVE_MAX_PATH];
    char         testPath[FS_NATIVE_MAX_PATH];
    const char  *baseDir;
    fileHandle_t f;
    int          len;

    if ( vmType ) *vmType = VMI_NONE;
    if ( !basename || !*basename || !outPath || outSize <= 0 ) {
        return qfalse;
    }
    ...
    if ( FS_NativeFindFile( basename, outPath, outSize ) ) {
        if ( vmType ) *vmType = VMI_NATIVE;
        Com_Printf( "FS_FindVM: native library '%s' bevorzugt\n", outPath );
        return qtrue;
    }

    /* 2) QVM-Fallback */
    baseDir = fs_basegame ? fs_basegame->string : BASEGAME;
    Com_sprintf( qvmPath, sizeof( qvmPath ), "%s/%s.qvm", baseDir, basename );
    len = FS_FOpenFileRead( qvmPath, &f, qfalse );
    if ( len >= 0 ) {
        if ( f ) FS_FCloseFile( f );
        Q_strncpyz( outPath, qvmPath, outSize );
        if ( vmType ) *vmType = VMI_BYTECODE;
        Com_Printf( "FS_FindVM: QVM fallback '%s'\n", qvmPath );
        return qtrue;
    }

    /* 3) Alternatives Host-Naming ohne fs_basegame-Präfix */
    Com_sprintf( testPath, sizeof( testPath ), "%s.qvm", basename );
    len = FS_FOpenFileRead( testPath, &f, qfalse );
    if ( len >= 0 ) {
        if ( f ) FS_FCloseFile( f );
        Q_strncpyz( outPath, testPath, outSize );
    ...
    Com_DPrintf( "FS_FindVM: no VM for '%s'\n", basename );
    return qfalse;
}
                                 ...
                                 char *out, int outSize )
{
    ...
    baseDir = fs_basegame ? fs_basegame->string : BASEGAME;
    Com_sprintf( out, outSize, "%s/%s.so", baseDir, basename );
}

unsigned FS_ServerNativeChecksum( const char *basename )
{
    char     nativePath[FS_NATIVE_MAX_PATH];
    void    *buf = NULL;
    int      len;
    unsigned sum = 0;

    if ( !basename || !*basename ) return 0;
    if ( !FS_NativeFindFile( basename, nativePath, sizeof( nativePath ) ) ) {
        Com_DPrintf( "FS_ServerNativeChecksum: no native '%s'\n", basename );
        return 0;
    }

    len = FS_ReadFile( nativePath, &buf );
    if ( len <= 0 || !buf ) {
        Com_DPrintf( "FS_ServerNativeChecksum: read failed '%s'\n",
                     nativePath );
        return 0;
    }

    sum = Com_BlockChecksum( buf, len );
    FS_FreeFile( buf );

    Com_DPrintf( "FS_ServerNativeChecksum: '%s' -> %08x\n",
                 nativePath, sum );
    return sum;
}

int FS_NativePakChecksumToString( const char *basename,
                                  char *buffer, int bufferSize )
{
    unsigned sum;

    if ( !buffer || bufferSize < FS_NATIVE_CHECKSUM_HEX ) {
        if ( buffer && bufferSize > 0 ) buffer[0] = '\0';
        return 0;
    }

    sum = FS_ServerNativeChecksum( basename );
    if ( sum == 0 ) {
        buffer[0] = '\0';
        return 0;
    }

    Com_sprintf( buffer, bufferSize, "@%08x", sum );
    return (int)strlen( buffer );
}
/* ============================================================ */
'''

    anchor = re.search(r'(qboolean\s+FS_FindVM\s*\(|void\s+FS_FindVM\s*\()',
                       content)
    if anchor:
        content = content[:anchor.start()] + helper + "\n" + content[anchor.start():]
    else:
    ...
    with open(files_c, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"[PATCHED] {files_c}: FS_FindVM + FS_NativeFindFile + "
          f"FS_ServerNativeChecksum + FS_NativePakChecksumToString")
    touched = True

    # --- FS_ReferencedPakPureChecksums erweitern ---
    ref_pat = re.compile(
        r'(\w+\s+FS_ReferencedPakPureChecksums\s*\([^)]*\)\s*\{)')
    if ref_pat.search(content):
        start = ref_pat.search(content).start()
        ...
        insert_at = j - 1
        inject = (
            "\n\t/* [PATCHED v13.7] Native Server-Pure-Checksumme */\n"
            "\t{\n"
            "\t\tchar nativeCs[FS_NATIVE_CHECKSUM_HEX];\n"
            "\t\tif ( FS_NativePakChecksumToString( \"smokinguns\",\n"
            "\t\t                                     nativeCs,\n"
            "\t\t                                     sizeof( nativeCs ) ) > 0 ) {\n"
            "\t\t\tQ_strcat( buffer, bufferSize, \" \" );\n"
            "\t\t\tQ_strcat( buffer, bufferSize, nativeCs );\n"
            "\t\t}\n"
            "\t}\n"
        )
        ...
        with open(files_c, "w", encoding="utf-8") as f:
            f.write(content)
        print("[PATCHED] FS_ReferencedPakPureChecksums: "
              "native Checksumme angehängt")
        touched = True
    else:
        print("[INFO] FS_ReferencedPakPureChecksums: Hook übersprungen")

    # --- VM_Create in vm.c hooken ---
    vm_c = None
    for root, _dirs, files in os.walk("code"):
    ...
    if vm_c:
        with open(vm_c, "r", encoding="utf-8", errors="ignore") as f:
            vc = f.read()
        if "FS_FindVM" not in vc:
            extern_decl = (
                "\nextern qboolean FS_FindVM( const char *basename, "
                "char *outPath, int outSize, vmType_t *vmType );\n"
            )
            inc_matches = list(re.finditer(r'#include\s+"[^"]+"\s*\n', vc))
            if inc_matches:
                pos = inc_matches[-1].end()
                vc = vc[:pos] + extern_decl + vc[pos:]
            m = re.search(r'(vm_t\s*\*\s*VM_Create\s*\([^)]*\)\s*\{)', vc)
            if m:
                hook = (
                    "\n\t/* [PATCHED v13.7] Native .so vor QVM bevorzugen */\n"
                    "\t{\n"
                    "\t\tchar      nativePath[4096];\n"
                    "\t\tvmType_t  nativeType = VMI_NONE;\n"
                    "\t\tif ( FS_FindVM( moduleName, nativePath,\n"
                    "\t\t                sizeof( nativePath ), &nativeType ) &&\n"
                    "\t\t     nativeType == VMI_NATIVE ) {\n"
                    "\t\t\tCom_Printf( \"VM_Create: using native '%s'\\n\",\n"
                    "\t\t\t            nativePath );\n"
                    "\t\t}\n"
                    "\t}\n"
                )
                vc = vc[:m.end()] + hook + vc[m.end():]
                with open(vm_c, "w", encoding="utf-8") as f:
                    f.write(vc)
                print(f"[PATCHED] {vm_c}: VM_Create Server-Native-Hook")
                touched = True
...
def main():
    ...
    applied = 0
    if is_smokinguns:
        print("==> Patching SmokinGuns (v13.5)")
        diagnostic_dump()
        ...
        patch_fs_native_vm()            # v13.6 Client Native VM
        patch_server_native_vm()        # v13.7 Server Native VM
        write_menu_override()
