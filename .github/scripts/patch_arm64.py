...
[EXTENDED] Handheld aim assist v7 mit Unlagged-Sync:
  - 11 CVARs (Master, Fire, Angle, Strength, Friction, Pitch, Lead,
    Sticky, MaxDist, LOS, UnlaggedSync)
  - Automatische Lead-Berechnung basierend auf dem Ping
  - Menü-Integration: Basic + Advanced Untermenü
"""
...
def patch_client_cvar():
    ...
    with open(path, "r", encoding="utf-8", errors="ignore") as f:
        content = f.read()
    if 'cg_handheldAimAssistUnlaggedSync' in content:
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"[PATCHED] cl_main.c: 11 cvars registered")
    return True
...
def patch_ui_cvar():
    ...
    if ui_local_path:
        with open(ui_local_path, "r", encoding="utf-8", errors="ignore") as f:
            content = f.read()
        if "ui_handheldAimAssistUnlaggedSync" not in content:
            ...
            if anchor in content:
                ...
                with open(ui_local_path, "w", encoding="utf-8") as f:
                    f.write(content)
                print("[PATCHED] ui_local.h: 11 extern decls added")
    ...
    if "ui_handheldAimAssistUnlaggedSync;" not in content:
        ...
        if anchor in content:
            ...
            content = content.replace(anchor, decls + anchor, 1)
    if '"cg_handheldAimAssistUnlaggedSync"' not in content:
    ...
    with open(ui_main_path, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"[PATCHED] ui_main.c: 11 cvars registered")
    return True
...
def patch_aim_assist():
    ...
    helper_code = r"""
/* ============================================================
 * [PATCHED] Handheld Aim Assist v7 mit Unlagged-Sync
 * ============================================================ */
#include <math.h>
...
static int HHA_FindTarget( const hha_config_t *cfg, vec3_t bestDir, float *bestAngleOut ) {
    ...
    *bestAngleOut = 999.0f;
    ...
    for ( i = 0; i < cl.snap.numEntities; i++ ) {
        ...
        float leadTime = cfg->leadTime;

        if ( ent->eType != ET_PLAYER ) continue;
        if ( ent->number == cl.snap.ps.clientNum ) continue;
        if ( ent->eFlags & EF_DEAD ) continue;

        /* Unlagged-Sync: Wenn aktiviert, nutze den halben Ping als Lead-Zeit */
        if (cfg->unlaggedSync) {
            leadTime = (float)cl.snap.ping * 0.0005f;
            if (leadTime > 0.15f) leadTime = 0.15f;
        }

        HHA_PredictPos( ent, leadTime, predFeet );
...
"""
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"[PATCHED] Aim assist v7 in {target_file}")
    return True
...
itemDef {
	name aim_hint1
	style 1
	text "Advanced: Lead-Prediction, Pitch-Strength, Sticky, etc."
	rect 60 210 520 16
	textalign ITEM_ALIGN_CENTER
	textalignx 260
	textaligny 12
...
itemDef {
	name aim_hint2
	style 1
	text "Unlagged-Sync is recommended for online play."
	rect 60 227 520 16
	textalign ITEM_ALIGN_CENTER
	textalignx 260
	textaligny 12
...
itemDef {
	name aim_subtitle
	style 1
	text "Unlagged-Sync uses your ping to aim ahead automatically."
	rect 60 32 520 18
	textalign ITEM_ALIGN_CENTER
	textalignx 260
	textaligny 14
...
itemDef {
	name aim_hint1
	style 1
	text "If Unlagged-Sync is Yes, Manual Lead is ignored."
	rect 60 215 520 16
	textalign ITEM_ALIGN_CENTER
	textalignx 260
	textaligny 12
...
itemDef {
	name aim_hint2
	style 1
	text "Pitch: vertical pull. LOS: don't aim through walls."
	rect 60 232 520 16
	textalign ITEM_ALIGN_CENTER
	textalignx 260
	textaligny 12
