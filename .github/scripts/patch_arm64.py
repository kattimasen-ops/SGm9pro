...
v13.1 - Aim-Assist Stabilitaet + Hitmarker + Crosshair + In-Game-Menue:
  - EnsureCvars ueberschreibt Nutzereinstellungen nicht mehr
  - Respawn-Reset reagiert auch auf Health-Wechsel (map_restart safe)
  - Hitmarker-Feld-Semantik korrigiert (otherEntityNum statt eventParm)
  - Crosshair wird orange bei Ziel auf gegnerisches Playermodel
  - Aim-Assist-Button in code/ui/ingame_options.menu an Y=255 eingefuegt
"""
...
def patch_client_cvar():
    ...
    lines = [
        "\t/* [PATCHED v13] Handheld Aim Assist cvars */",
...
def patch_aim_assist():
    ...
    helper_code = r"""
/* ============================================================
 * [PATCHED v13.1] Handheld Aim Assist
...
"""
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"[PATCHED] Aim assist v13.1 in {target_file}")
    return True
...
def patch_cg_view():
    ...
    zoom_helper = r"""
/* ============================================================
 * [PATCHED v13.1] Dynamic Zoom + Auto-Switch
 * ============================================================ */
...
def patch_cg_hitmarker():
    ...
    hitmarker_code = r"""
/* ============================================================
 * [PATCHED v13.1] Hitmarker + Damage-Indicator
 * ============================================================ */
...
def patch_cg_event_hit():
    ...
    if bullet_case.search(content):
        # KORREKTUR v13.1:
        # In Smokin' Guns ist es->otherEntityNum der Schuetze.
        # es->eventParm ist die Richtung (ByteToDir) UND die Entity-Nummer
        # des Getroffenen — die beiden Wertebereiche ueberlappen.
        # Korrekt: nur otherEntityNum pruefen.
        content = bullet_case.sub(
            ...
            r'\t\t}',
            content, count=1)
        print("[PATCHED] Hitmarker event hook on EV_BULLET_HIT_FLESH (fixed)")
...
def patch_cg_crosshair_player():
    """FIX v13.1: Crosshair wird orange bei Ziel auf gegnerisches Playermodel."""
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_draw.c" in files:
            target_file = os.path.join(root, "cg_draw.c")
...
def patch_ingame_options_menu():
    """FIX v13.1: Fuegt einen Button zum Aim-Assist-Menue in ingame_options.menu ein.

    Datei liegt im Source-Tree unter code/ui/ingame_options.menu.
    Position: rechte Spalte (ROW2=300), Y=255. Die naechste freie Zeile nach
    'Glowing Flares' (Y=235, H=18, endet bei Y=253) im 300px hohen Menue.
    """
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "ingame_options.menu" in files:
            ...
            break
    if not target_file:
        print("[WARN] ingame_options.menu not found in code/ui/")
        return False
...
'''
    ...
    # Struktur der Datei:  { ... menuDef { ... items ... } }
    # Die vorletzte schliessende Klammer ist die des menuDef.
...
def main():
    ...
    applied = 0
    if is_smokinguns:
        print("==> Patching SmokinGuns (v13.1)")
        diagnostic_dump()
