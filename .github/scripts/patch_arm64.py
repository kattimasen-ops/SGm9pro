...
v14.1 - Aim-Assist: Target-Scan nach Input.
...
v14.5 - Preset-Buttons: setcvar statt exec.
v14.6 - EnemyOutline-Cvars in AIM_CVAR_SUFFIXES.
v14.7 - Aim-Assist: EF_NODRAW, Map-Bounds, MASK_PLAYERSOLID.
v14.8 - Manuelles Entsperren, Auto-Unlock, Lock-Timeout.
v14.9 - Crosshair Min 25, Damage-Indicator groesser,
        Rename-Centerprint-Filter.
v15.0 - Client-seitiger Gegner-Radar.
v15.1 - Kompass, Low-Health-Vignette, Kill-Chat-Filter,
        Kill-Feed nach oben links.
v15.2 - Treffer-Meldungen Filter (Hitmessage).
v15.3 - Radar Redesign: Kompass-Dots + Edge-Indicators.
v15.4 - Mini-Radar im Top-Right.
v15.5 - Mini-Radar mit Priority-Highlight, Damage-Spike
        und Off-Screen-Arrows.
v15.5-fix2 - HHA_SHARED_BLOCK mit Preprocessor-Guard:
             hha_threat_t, hha_threatCache und Helper werden
             in jedem der drei Patches (radar, mini_radar,
             compass) vorangestellt, damit die Reihenfolge der
             Aufrufe keine Rolle mehr spielt.
"""
...
AIM_CVAR_SUFFIXES = [
    ...
    "AimPreset", "EnemyOutline", "EnemyOutlineRange",
    "Radar", "RadarMode", "RadarRange", "RadarEdgeSize", "MiniRadar",
    "HideKillChat", "Compass", "LowHealthVignette",
]


HHA_SHARED_BLOCK = r'''
/* ===== HHA_THREAT_SHARED_BLOCK ===== */
#ifndef HHA_THREAT_SHARED_BLOCK
#define HHA_THREAT_SHARED_BLOCK

#include <math.h>

#ifndef HHA_THREAT_MAX_ENTITIES
#define HHA_THREAT_MAX_ENTITIES   64
#endif
#ifndef HHA_SND_TIMEOUT
#define HHA_SND_TIMEOUT           3000
#endif
#ifndef HHA_THREAT_LOS_BUDGET
#define HHA_THREAT_LOS_BUDGET     8
#endif
#ifndef HHA_THREAT_MIN_DIST
#define HHA_THREAT_MIN_DIST       64.0f
#endif
#ifndef HHA_EDGE_SAFE_MARGIN
#define HHA_EDGE_SAFE_MARGIN      8.0f
#endif

#ifndef HHA_SND_LEVEL_T_DEFINED
#define HHA_SND_LEVEL_T_DEFINED
typedef enum {
    HHA_SND_NONE = 0,
    HHA_SND_QUIET   = 1,
    HHA_SND_NORMAL  = 2,
...
#ifndef HHA_THREAT_T_DEFINED
#define HHA_THREAT_T_DEFINED
typedef struct {
    int    entNum;
    float  distance;
    float  bearing;
...
#ifndef HHA_THREAT_STATE_DEFINED
#define HHA_THREAT_STATE_DEFINED

static hha_threat_t hha_threatCache[HHA_THREAT_MAX_ENTITIES];
static int          hha_threatNum = 0;

static int      hha_sndLastTime[MAX_GENTITIES];
static int      hha_sndLevel[MAX_GENTITIES];
static qboolean hha_sndLooping[MAX_GENTITIES];
static vec3_t   hha_sndLastPos[MAX_GENTITIES];

static float HHA_Threat_CvarFloat( const char *name ) {
    char buf[16];
    buf[0] = 0;
...
def patch_client_cvar():
    ...
    lines = [
        ...
        "\t/* [PATCHED v15.5] Radar cvars */",
        "\tCvar_Get (\"cg_handheldRadar\", \"1\", CVAR_ARCHIVE);",
        "\tCvar_Get (\"cg_handheldRadarMode\", \"3\", CVAR_ARCHIVE);",
        "\tCvar_Get (\"cg_handheldRadarRange\", \"3000\", CVAR_ARCHIVE);",
        "\tCvar_Get (\"cg_handheldRadarEdgeSize\", \"20\", CVAR_ARCHIVE);",
        "\tCvar_Get (\"cg_handheldMiniRadar\", \"1\", CVAR_ARCHIVE);",
        "\t/* [PATCHED v15.1] HUD cvars */",
        "\tCvar_Get (\"cg_handheldHideKillChat\", \"1\", CVAR_ARCHIVE);",
        "\tCvar_Get (\"cg_handheldCompass\", \"1\", CVAR_ARCHIVE);",
        "\tCvar_Get (\"cg_handheldLowHealthVignette\", \"1\", CVAR_ARCHIVE);",
    ]
    content = content.replace(anchor, anchor + "\n" + "\n".join(lines), 1)
    with open(path, "w", encoding="utf-8") as f:
        f.write(content)
    print("[PATCHED] cl_main.c: 40 cvars registered")
    return True
    ...
    if '"cg_handheldAutoSwitch"' not in content:
        ...
        if anchor in content:
            entries = (
                ...
                '\n\t{ &ui_handheldRadar, "cg_handheldRadar", "1", CVAR_ARCHIVE },'
                '\n\t{ &ui_handheldRadarMode, "cg_handheldRadarMode", "3", CVAR_ARCHIVE },'
                '\n\t{ &ui_handheldRadarRange, "cg_handheldRadarRange", "3000", CVAR_ARCHIVE },'
                '\n\t{ &ui_handheldRadarEdgeSize, "cg_handheldRadarEdgeSize", "20", CVAR_ARCHIVE },'
                '\n\t{ &ui_handheldMiniRadar, "cg_handheldMiniRadar", "1", CVAR_ARCHIVE },'
                '\n\t{ &ui_handheldHideKillChat, "cg_handheldHideKillChat", "1", CVAR_ARCHIVE },'
                '\n\t{ &ui_handheldCompass, "cg_handheldCompass", "1", CVAR_ARCHIVE },'
                '\n\t{ &ui_handheldLowHealthVignette, "cg_handheldLowHealthVignette", "1", CVAR_ARCHIVE },'
            )
            content = content.replace(anchor, anchor + entries, 1)
            print("[PATCHED] ui_main.c: 33 cvarTable entries")
...
def patch_aim_assist():
    ...
    helper_code = r"""
/* ============================================================
 * [PATCHED v14.8] Handheld Aim Assist (Client-Seite)
 * ============================================================ */
#include <math.h>
...
#define HHA_MAP_BOUND       4096.0f
#define HHA_UNLOCK_MOVE_THRESHOLD 15.0f
#define HHA_LOCK_TIMEOUT_MS 500
...
static int hha_lockAcquiredTime = 0;
static int hha_lockLastVisibleTime = 0;
...
static qboolean HHA_HasLineOfSight( int entNum, const vec3_t fromFeet, const vec3_t toFeet ) {
    ...
    CM_BoxTrace( &tr, eye, chest, NULL, NULL, 0, MASK_PLAYERSOLID, 0 );
...
static qboolean HHA_CheckManualUnlock( void ) {
    int dx = cl.mouseDx[cl.mouseIndex];
    int dy = cl.mouseDy[cl.mouseIndex];
    float mag;

    if ( dx == 0 && dy == 0 ) return qfalse;
    mag = sqrtf( (float)(dx * dx + dy * dy) );
    if ( mag >= HHA_UNLOCK_MOVE_THRESHOLD ) {
        HHA_Log("Manual unlock: joystick move mag=%.1f", mag);
        return qtrue;
    }
    return qfalse;
}
...
static int HHA_FindTarget( const hha_config_t *cfg, vec3_t bestDir, float *bestAngleOut ) {
    ...
    *bestAngleOut = 999.0f;
    ...
    for ( i = 0; i < cl.snap.numEntities && ncands < HHA_MAX_CANDIDATES; i++ ) {
        ...
        if ( ent->eFlags & (EF_DEAD | EF_NODRAW) ) continue;

        HHA_GetPredictedPos( ent, cfg, predFeet );

        if ( predFeet[0] < -HHA_MAP_BOUND || predFeet[0] > HHA_MAP_BOUND ||
             predFeet[1] < -HHA_MAP_BOUND || predFeet[1] > HHA_MAP_BOUND ||
             predFeet[2] < -HHA_MAP_BOUND || predFeet[2] > HHA_MAP_BOUND ) {
            continue;
        }
...
static void CL_HandheldRespawnCheck( void ) {
    ...
    if ( respawned ) {
        ...
        hha_lastTargetNum = -1;
        hha_lastTargetAge = 0;
        hha_lockAcquiredTime = 0;
        hha_lockLastVisibleTime = 0;
        for ( i = 0; i < HHA_LOS_CACHE_SIZE; i++ )
...
static void CL_HandheldSearchTarget( void ) {
    ...
    if ( hha_cachedTarget >= 0 ) {
        ...
        hha_lastTargetAge = 0;
        hha_lockLastVisibleTime = cl.serverTime;
        if ( hha_lockAcquiredTime == 0 ) {
            hha_lockAcquiredTime = cl.serverTime;
            HHA_Log("Lock acquired: target=%d", hha_cachedTarget);
        }
    } else if ( ++hha_lastTargetAge > hha_cachedConfig.stickyFrames ) {
        if ( hha_lastTargetNum >= 0 ) {
            HHA_Log("Lock released: target=%d (age=%d)",
                    hha_lastTargetNum, hha_lastTargetAge);
        }
        hha_lastTargetNum = -1;
        hha_lockAcquiredTime = 0;
    }
...
static void CL_HandheldAimMagnetism( usercmd_t *cmd ) {
    ...
    if ( cfg->hardLock && hha_lockLastVisibleTime > 0 ) {
        int timeSinceVisible = cl.serverTime - hha_lockLastVisibleTime;
        if ( timeSinceVisible > HHA_LOCK_TIMEOUT_MS ) {
            HHA_Log("Lock timeout: no LoS for %d ms", timeSinceVisible);
            hha_cachedTarget = -1;
            hha_cachedValid  = qfalse;
            hha_lastTargetNum = -1;
            hha_lockAcquiredTime = 0;
...
"""
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"[PATCHED] Aim assist v14.8 in {target_file}")
    return True
...
static qboolean HHA_NameRotator_JustRespawned( void ) {
    ...
    if ( !cl.snap.valid ) return qfalse;
    ...
    if ( respawned && curHealth > 0 && curPmType != PM_DEAD ) return qtrue;
    return qfalse;
}
...
'''
    ...
    if n == 0:
        print("[WARN] Chat prefill: kein Hook eingebaut")
        return False
...
def patch_cg_crosshair_min_size():
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_draw.c" in files:
            ...
            break
    if not target_file or not os.path.exists(target_file):
        print("[WARN] cg_draw.c not found (crosshair min-size)")
        return False
    with open(target_file, "r", encoding="utf-8", errors="ignore") as f:
        content = f.read()

    if "HHA_CROSSHAIR_MIN_SIZE" in content:
        print("[SKIP] Crosshair min-size already patched")
        return True

    pattern = re.compile(r'(\n[ \t]*w\s*=\s*h\s*=\s*cg_crosshairSize\.value\s*;)')
    if not pattern.search(content):
        print("[WARN] cg_crosshairSize assignment not found")
        return False

    content, n = pattern.subn(
        r'\1\n'
        r'\t/* HHA_CROSSHAIR_MIN_SIZE: auf R36S-Display mind. 25 */\n'
        r'\tif ( w < 25.0f ) w = h = 25.0f;',
        content, count=1)
    if n == 0:
        return False
    print("[PATCHED] Crosshair Mindestgroesse 25")

    pattern3d = re.compile(r'(\n[ \t]*w\s*=\s*cg_crosshairSize\.value\s*;)')
    if pattern3d.search(content):
        content, n3 = pattern3d.subn(
            r'\1\n'
            r'\t/* HHA_CROSSHAIR_MIN_SIZE (3D) */\n'
            r'\tif ( w < 25.0f ) w = 25.0f;',
            content, count=1)
        if n3 > 0:
            print("[PATCHED] Crosshair Mindestgroesse 25 (CG_DrawCrosshair3D)")
...
def patch_cg_hitmarker():
    ...
    if "HHA_HITMARKER_V4" in content:
        print("[SKIP] Hitmarker v14.9 bereits gepatcht")
        return True

    for old_marker in ["HHA_HITMARKER_V3", "HHA_HITMARKER_V2",
                       "[PATCHED v14.3] Hitmarker",
...
/* ============================================================
 * [PATCHED v14.9] Hitmarker + Damage-Indicator (Ring-Dots)
 * HHA_HITMARKER_V4
 * ============================================================ */
#include <math.h>
...
void CG_DrawDamageIndicator( void ) {
    ...
    if ( alpha > 0.3f ) {
        CG_DrawPic( cx - 3.0f, cy - 3.0f, 6.0f, 6.0f, cgs.media.whiteShader );
    }
...
'''
    ...
    if draw2d_pat.search(content):
        content = draw2d_pat.sub(
            r'\1\n\n\t/* HHA_HITMARKER_V4 */\n\tCG_HandheldDrawOverlays();',
            content, count=1)
        print("[PATCHED] Hitmarker + Damage-Indicator v14.9")
...
def patch_cg_hha_radar():
    """v15.3: Threat-Daten + Edge-Indicators. Enthaelt HHA_SHARED_BLOCK."""
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_draw.c" in files:
            target_file = os.path.join(root, "cg_draw.c")
            break
    if not target_file or not os.path.exists(target_file):
        print("[WARN] cg_draw.c not found (radar)")
        return False

    with open(target_file, "r", encoding="utf-8", errors="ignore") as f:
        content = f.read()

    if "HHA_RADAR_V3" in content:
        print("[SKIP] Radar v15.3 bereits gepatcht")
        return True

    if "HHA_RADAR_V1" in content:
        idx = content.find("HHA_RADAR_V1")
        ...
        if start >= 0:
            ...
            if end >= 0:
                ...
                content = content[:start] + content[end:]
                print("[REMOVED] alter HHA_RADAR_V1 Block")

    radar_code = HHA_SHARED_BLOCK + r'''

/* ============================================================
 * [PATCHED v15.3] Threat-Daten + Edge-Indicators
 * HHA_RADAR_V3
 * ============================================================ */

void CG_HHA_TrackSound( int entityNum, int level ) {
    if ( entityNum < 0 || entityNum >= MAX_GENTITIES ) return;
    if ( level <= 0 ) return;
    hha_sndLastTime[entityNum] = cg.time;
    if ( level > hha_sndLevel[entityNum] ) {
        hha_sndLevel[entityNum] = level;
    } else {
        hha_sndLevel[entityNum] = (hha_sndLevel[entityNum] * 3 + level) / 4;
    }
    VectorCopy( cg_entities[entityNum].lerpOrigin, hha_sndLastPos[entityNum] );
}

void CG_HHA_TrackLoop( int entityNum, qboolean active ) {
    if ( entityNum < 0 || entityNum >= MAX_GENTITIES ) return;
    hha_sndLooping[entityNum] = active;
    if ( active ) {
        hha_sndLastTime[entityNum] = cg.time;
        VectorCopy( cg_entities[entityNum].lerpOrigin, hha_sndLastPos[entityNum] );
    }
...
static float HHA_Threat_SoundStrength( int entNum ) {
    int dt;
    float fade;
    if ( entNum < 0 || entNum >= MAX_GENTITIES ) return 0.0f;
    dt = cg.time - hha_sndLastTime[entNum];
    if ( dt < 0 || dt > HHA_SND_TIMEOUT ) return 0.0f;
    fade = 1.0f - ( (float)dt / HHA_SND_TIMEOUT );
    return fade * ( (float)hha_sndLevel[entNum] / (float)HHA_SND_VLOUD );
}

static qboolean HHA_Threat_IsEnemy( entityState_t *ent ) {
    if ( ent->eType != ET_PLAYER ) return qfalse;
    if ( ent->number == cg.snap->ps.clientNum ) return qfalse;
    if ( ent->eFlags & (EF_DEAD | EF_NODRAW) ) return qfalse;
...
void CG_HHA_Radar_Update( void ) {
    ...
    if ( HHA_Threat_CvarFloat("cg_handheldRadar") == 0.0f ) return;
    mode = (int)HHA_Threat_CvarFloat("cg_handheldRadarMode");
    if ( mode == 0 ) return;

    maxDist = HHA_Threat_CvarFloat("cg_handheldRadarRange");
    if ( maxDist < 500.0f )  maxDist = 500.0f;
    if ( maxDist > 8000.0f ) maxDist = 8000.0f;

    playerYaw = cg.refdefViewAngles[YAW];
    while ( playerYaw < 0.0f ) playerYaw += 360.0f;
    while ( playerYaw >= 360.0f ) playerYaw -= 360.0f;

    for ( i = 0; i < cg.snap->numEntities && hha_threatNum < HHA_THREAT_MAX_ENTITIES; i++ ) {
        entityState_t *ent = &cg.snap->entities[i];
        centity_t     *cent = &cg_entities[ent->number];
        hha_threat_t  *t;
        float distSq, bearingRad;

        if ( !HHA_Threat_IsEnemy( ent ) ) continue;

        VectorSubtract( cent->lerpOrigin, cg.refdef.vieworg, delta );
        distSq = delta[0]*delta[0] + delta[1]*delta[1] + delta[2]*delta[2];
        if ( distSq < HHA_THREAT_MIN_DIST * HHA_THREAT_MIN_DIST ) continue;
        if ( distSq > maxDist * maxDist ) continue;

        t = &hha_threatCache[hha_threatNum];
        t->entNum = ent->number;
        t->distance = sqrtf( distSq );
        t->isLooping = hha_sndLooping[ent->number];
        t->soundStrength = HHA_Threat_SoundStrength( ent->number );
        VectorCopy( cent->lerpOrigin, t->worldPos );

        bearingRad = atan2f( delta[0], delta[1] );
        t->bearing = bearingRad * ( 180.0f / M_PI );
        if ( t->bearing < 0.0f ) t->bearing += 360.0f;

        t->relAngle = t->bearing - playerYaw;
        while ( t->relAngle > 180.0f ) t->relAngle -= 360.0f;
        while ( t->relAngle < -180.0f ) t->relAngle += 360.0f;

        t->hasLoS = qfalse;
        if ( HHA_THREAT_LOS_BUDGET > 0 ) {
            trace_t tr;
            vec3_t eye, chest;
            VectorCopy( cg.refdef.vieworg, eye );
            VectorCopy( t->worldPos, chest );
            chest[2] += 32.0f;
            CG_Trace( &tr, eye, NULL, NULL, chest,
                      cg.snap->ps.clientNum, MASK_SHOT );
            t->hasLoS = ( tr.fraction >= 0.999f );
        }
...
void CG_HHA_DrawEdgeIndicators( void ) {
    int i;
    float fovHalf;
    int mode;
    float edgeSize, halfEdge;
    ...
    edgeSize = HHA_Threat_CvarFloat("cg_handheldRadarEdgeSize");
    if ( edgeSize < 12.0f ) edgeSize = 12.0f;
    if ( edgeSize > 40.0f ) edgeSize = 40.0f;
    halfEdge = edgeSize * 0.5f;

    fovHalf = cg.refdef.fov_x * 0.5f;
    if ( fovHalf < 30.0f ) fovHalf = 30.0f;
    if ( fovHalf > 80.0f ) fovHalf = 80.0f;

    for ( i = 0; i < hha_threatNum; i++ ) {
        hha_threat_t *t = &hha_threatCache[i];
        float rel = t->relAngle;
        float absRel = fabsf( rel );
        float alpha;
        float col[4];

        if ( mode == 3 && absRel <= fovHalf ) continue;

        alpha = 0.55f + t->soundStrength * 0.45f;
        if ( alpha > 1.0f ) alpha = 1.0f;

        if ( t->hasLoS ) {
            col[0] = 1.0f; col[1] = 0.20f; col[2] = 0.10f;
        } else if ( t->soundStrength > 0.4f ) {
            col[0] = 1.0f; col[1] = 0.65f; col[2] = 0.10f;
        } else if ( t->soundStrength > 0.05f || t->isLooping ) {
            col[0] = 0.85f; col[1] = 0.85f; col[2] = 0.20f;
        } else {
            col[0] = 0.55f; col[1] = 0.55f; col[2] = 0.55f;
        }
        col[3] = alpha;

        if ( rel >= -135.0f && rel < -fovHalf ) {
            float tt = ( -rel - fovHalf ) / ( 135.0f - fovHalf );
            float y = 100.0f + tt * 280.0f;
            trap_R_SetColor( col );
            CG_DrawPic( HHA_EDGE_SAFE_MARGIN, y - halfEdge,
                        halfEdge, edgeSize, cgs.media.indicate_lf );
            trap_R_SetColor( NULL );
        } else if ( rel > fovHalf && rel <= 135.0f ) {
            float tt = ( rel - fovHalf ) / ( 135.0f - fovHalf );
            float y = 100.0f + tt * 280.0f;
            trap_R_SetColor( col );
            CG_DrawPic( 640.0f - HHA_EDGE_SAFE_MARGIN - halfEdge, y - halfEdge,
                        halfEdge, edgeSize, cgs.media.indicate_rt );
            trap_R_SetColor( NULL );
        } else {
            float x;
            if ( rel < 0.0f ) {
                float tt = ( -rel - 135.0f ) / 45.0f;
                x = 320.0f - tt * 260.0f;
            } else {
                float tt = ( rel - 135.0f ) / 45.0f;
                x = 320.0f + tt * 260.0f;
            ...
            trap_R_SetColor( col );
            CG_DrawPic( x - halfEdge, 480.0f - HHA_EDGE_SAFE_MARGIN - halfEdge,
                        edgeSize, halfEdge, cgs.media.indicate_bk );
            trap_R_SetColor( NULL );
        }
    }
}
...
'''
    ...
    inc_pat = re.compile(r'(#include\s+"cg_local\.h"\s*\n)')
    if not inc_pat.search(content):
        print("[WARN] cg_draw.c: cg_local.h include nicht gefunden")
        return False
    content = inc_pat.sub(lambda m: m.group(1) + radar_code, content, count=1)

    hook_pat = re.compile(
        r'(\n[ \t]*CG_DrawCrosshairNames\s*\(\s*isPlayer\s*\)\s*;)'
    )
    if hook_pat.search(content):
        content = hook_pat.sub(
            r'\1\n\n\t/* HHA_RADAR_V3 */\n'
            r'\tCG_HHA_Radar_Update();\n'
            r'\tCG_HHA_DrawEdgeIndicators();',
            content, count=1
        )
        print("[PATCHED] Radar v15.3 Hook")
...
def patch_cg_ents_sound_hook():
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_ents.c" in files:
            target_file = os.path.join(root, "cg_ents.c")
            break
    if not target_file or not os.path.exists(target_file):
        print("[WARN] cg_ents.c not found")
        return False

    with open(target_file, "r", encoding="utf-8", errors="ignore") as f:
        content = f.read()

    if "HHA_ENTS_SOUND_TRACK" in content:
        print("[SKIP] cg_ents.c Sound-Hook bereits vorhanden")
        return True

    decl_block = (
        "\n/* HHA_ENTS_SOUND_TRACK */\n"
        "extern void CG_HHA_TrackSound( int entityNum, int level );\n"
        "extern void CG_HHA_TrackLoop( int entityNum, qboolean active );\n\n"
    )
    inc_pat = re.compile(r'(#include\s+"cg_local\.h"\s*\n)')
    if inc_pat.search(content):
        content = inc_pat.sub(lambda m: m.group(1) + decl_block, content, count=1)

    hook_pat = re.compile(r'(\tCG_SetEntitySoundPosition\(\s*cent\s*\)\s*;)')
    if not hook_pat.search(content):
        print("[WARN] cg_ents.c: CG_SetEntitySoundPosition nicht gefunden")
        return False

    content = hook_pat.sub(
        r'\1\n\n\t/* HHA_ENTS_SOUND_TRACK */\n'
        r'\tif ( cent->currentState.loopSound ) {\n'
        r'\t\tCG_HHA_TrackLoop( cent->currentState.number, qtrue );\n'
        r'\t} else {\n'
        r'\t\tCG_HHA_TrackLoop( cent->currentState.number, qfalse );\n'
        r'\t}',
        content, count=1
    )
    print("[PATCHED] cg_ents.c: Sound-Tracking")
...
def patch_cg_event_sound_hook():
    ...
    if "HHA_EVENT_SOUND_TRACK" in content:
        print("[SKIP] cg_event.c Sound-Hook bereits vorhanden")
        return True

    helper_code = r'''
/* ============================================================
 * [PATCHED v15.0] Event-Sound-Tracking fuer Radar
 * HHA_EVENT_SOUND_TRACK
 * ============================================================ */
extern void CG_HHA_TrackSound( int entityNum, int level );

#define HHA_SND_NONE     0
#define HHA_SND_QUIET    1
#define HHA_SND_NORMAL   2
#define HHA_SND_LOUD     3
#define HHA_SND_VLOUD    4

static int HHA_ClassifyEvent( int event ) {
    switch ( event ) {
        case EV_FIRE_WEAPON:
...
static void HHA_TrackEntityEvent( centity_t *cent, int event ) {
    int level;
    int num;

    level = HHA_ClassifyEvent( event );
    if ( level == HHA_SND_NONE ) return;

    num = cent->currentState.number;
    if ( num < 0 || num >= MAX_GENTITIES ) return;

    CG_HHA_TrackSound( num, level );
}
/* ============================================================ */
'''
    ...
    inc_pat = re.compile(r'(#include\s+"cg_local\.h"\s*\n)')
    if not inc_pat.search(content):
        print("[WARN] cg_event.c: cg_local.h include nicht gefunden")
        return False
    content = inc_pat.sub(lambda m: m.group(1) + helper_code, content, count=1)

    hook_pat = re.compile(
        r'(\tif\s*\(\s*!\s*event\s*\)\s*\{\s*\n'
        r'\t\tDEBUGNAME\("ZEROEVENT"\);\s*\n'
        r'\t\treturn;\s*\n'
        r'\t\})'
    )
    if not hook_pat.search(content):
        print("[WARN] cg_event.c: ZEROEVENT anchor nicht gefunden")
        return False

    content = hook_pat.sub(
        r'\1\n\n'
        r'\t/* HHA_EVENT_SOUND_TRACK */\n'
        r'\tHHA_TrackEntityEvent( cent, event );',
        content, count=1
    )
    print("[PATCHED] cg_event.c: Sound-Tracking")
...
def patch_cg_event_hit():
    ...
    if bullet_case.search(content):
        content = bullet_case.sub(
            ...
            r'\t\t}',
            content, count=1)
        print("[PATCHED] Hitmarker event hook")
    ...
    if death_pat.search(content):
        content = death_pat.sub(
            ...
            r'\t\t}',
            content, count=1)
        print("[PATCHED] Kill-Hitmarker event hook")
...
def patch_cg_servercmds_rename_filter():
    target_file = os.path.join("code", "cgame", "cg_servercmds.c")
    if not os.path.exists(target_file):
        for root, _dirs, files in os.walk("code"):
            if "cg_servercmds.c" in files:
                target_file = os.path.join(root, "cg_servercmds.c")
                break
    if not target_file or not os.path.exists(target_file):
        print("[WARN] cg_servercmds.c not found")
        return False
    with open(target_file, "r", encoding="utf-8", errors="ignore") as f:
        content = f.read()

    if "HHA_RENAME_CP_FILTER" in content:
        print("[SKIP] Rename centerprint filter already present")
        return True
    ...
    if old_block not in content:
        print("[WARN] cg_servercmds.c: cp-Block nicht gefunden")
        return False

    new_block = (
        '\tif ( !strcmp( cmd, "cp" ) ) {\n'
        '\t\t/* HHA_RENAME_CP_FILTER */\n'
        '\t\t{\n'
        '\t\t\tconst char *cpText = CG_Argv(1);\n'
        '\t\t\tconst char *myName = cgs.clientinfo[cg.snap->ps.clientNum].name;\n'
        '\t\t\tif ( cpText && myName && *cpText && *myName ) {\n'
        '\t\t\t\tchar strippedCp[256];\n'
        '\t\t\t\tchar strippedName[256];\n'
        '\t\t\t\tint  si, sj;\n'
        '\t\t\t\tsj = 0;\n'
        '\t\t\t\tfor ( si = 0; cpText[si] && sj < 255; si++ ) {\n'
        '\t\t\t\t\tif ( cpText[si] == \'^\' && cpText[si+1] && cpText[si+1] != \'^\' ) { si++; continue; }\n'
        '\t\t\t\t\tstrippedCp[sj++] = cpText[si];\n'
        '\t\t\t\t}\n'
        '\t\t\t\tstrippedCp[sj] = 0;\n'
        '\t\t\t\tsj = 0;\n'
        '\t\t\t\tfor ( si = 0; myName[si] && sj < 255; si++ ) {\n'
        '\t\t\t\t\tif ( myName[si] == \'^\' && myName[si+1] && myName[si+1] != \'^\' ) { si++; continue; }\n'
        '\t\t\t\t\tstrippedName[sj++] = myName[si];\n'
        '\t\t\t\t}\n'
        '\t\t\t\tstrippedName[sj] = 0;\n'
        '\t\t\t\tif ( strippedCp[0] && strippedName[0] &&\n'
        '\t\t\t\t     !Q_stricmp( strippedCp, strippedName ) ) {\n'
        '\t\t\t\t\treturn;\n'
        '\t\t\t\t}\n'
        '\t\t\t}\n'
        '\t\t}\n'
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print("[PATCHED] cg_servercmds.c: Rename-Centerprint-Filter")
    return True


def patch_hud_killfeed_filter():
    target_file = os.path.join("code", "cgame", "cg_servercmds.c")
    if not os.path.exists(target_file):
        for root, _dirs, files in os.walk("code"):
            if "cg_servercmds.c" in files:
                ...
                break
    if not target_file or not os.path.exists(target_file):
        print("[WARN] cg_servercmds.c not found (killfeed filter)")
        return False
    with open(target_file, "r", encoding="utf-8", errors="ignore") as f:
        content = f.read()

    if "HHA_KILLFEED_FILTER" in content:
        print("[SKIP] Killfeed-Filter bereits gepatcht")
        return True

    helper = r'''
/* [PATCHED v15.1] HHA_KILLFEED_FILTER */
static qboolean HHA_IsKillMessage( const char *text ) {
    const char *t;
    if ( !text || !*text ) return qfalse;
    t = text;
    if ( *t == '^' && *(t+1) ) t += 2;
    if ( strstr( t, " killed " ) != NULL && strstr( t, " with " ) != NULL ) return qtrue;
    if ( strstr( t, "teamkilled" ) != NULL ) return qtrue;
    return qfalse;
}

'''

    anchor = "static void CG_ServerCommand( void ) {"
    if anchor not in content:
        print("[WARN] cg_servercmds.c: CG_ServerCommand anchor fehlt")
        return False
    ...
    if old_chat not in content:
        print("[WARN] cg_servercmds.c: chat-Block nicht gefunden")
        return False
    ...
    new_chat = (
        ...
        '\t\t\tCG_RemoveChatEscapeChar( text );\n'
        '\t\t\t/* HHA_KILLFEED_FILTER */\n'
        '\t\t\tif ( Cvar_VariableIntegerValue("cg_handheldHideKillChat") &&\n'
        '\t\t\t     HHA_IsKillMessage( text ) ) return;\n'
        '\t\t\tif ( !cg_chatMode.integer ) {\n'
        '\t\t\t\tCG_Printf( "%s\\n", text );\n'
        '\t\t\t} else {\n'
        '\t\t\t\tCG_AddToChat( text );\n'
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print("[PATCHED] cg_servercmds.c: Kill-Chat-Filter")
    return True


def patch_hud_obituary_filter():
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_event.c" in files:
            target_file = os.path.join(root, "cg_event.c")
            break
    if not target_file or not os.path.exists(target_file):
        print("[WARN] cg_event.c not found (obituary filter)")
        return False
    with open(target_file, "r", encoding="utf-8", errors="ignore") as f:
        content = f.read()

    if "HHA_OBITUARY_FILTER" in content:
        print("[SKIP] Obituary-Filter bereits gepatcht")
        return True
    ...
    if old_block not in content:
        print("[WARN] cg_event.c: Obituary-Weapon-Block nicht gefunden")
        return False
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print("[PATCHED] cg_event.c: Obituary-Chat-Filter")
    return True


def patch_hud_hitmessage_filter():
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_event.c" in files:
            target_file = os.path.join(root, "cg_event.c")
            break
    if not target_file or not os.path.exists(target_file):
        print("[WARN] cg_event.c not found (hitmessage filter)")
        return False
    with open(target_file, "r", encoding="utf-8", errors="ignore") as f:
        content = f.read()

    if "HHA_HITMSG_FILTER" in content:
        print("[SKIP] Hitmessage-Filter bereits gepatcht")
        return True

    anchor = "static void CG_Hit_Message( entityState_t *ent ){"
    if anchor not in content:
        print("[WARN] cg_event.c: CG_Hit_Message anchor fehlt")
        return False

    guard = (
        "static void CG_Hit_Message( entityState_t *ent ){\n"
        "\t/* HHA_HITMSG_FILTER */\n"
        "\tif ( Cvar_VariableIntegerValue(\"cg_handheldHideKillChat\") ) return;\n"
    )
    content = content.replace(anchor, guard, 1)

    anchor2 = "static void CG_Hit_Far_Message( float ratio, int otherEntityNum ) {"
    if anchor2 in content:
        ...
        content = content.replace(anchor2, guard2, 1)
        print("[PATCHED] cg_event.c: CG_Hit_Far_Message gefiltert")

    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print("[PATCHED] cg_event.c: Hitmessage-Filter")
    return True


def patch_hud_killfeed_move_left():
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_draw.c" in files:
            target_file = os.path.join(root, "cg_draw.c")
            break
    if not target_file or not os.path.exists(target_file):
        print("[WARN] cg_draw.c not found (killfeed move)")
        return False
    with open(target_file, "r", encoding="utf-8", errors="ignore") as f:
        content = f.read()

    if "HHA_KILLFEED_LEFT" in content:
        print("[SKIP] Killfeed-Move bereits gepatcht")
        return True

    old_pos = '\t\t\tx = 640 - width - CHARMESS * (length[0]+length[1]);'
    new_pos = '\t\t\t/* HHA_KILLFEED_LEFT */\n\t\t\tx = 4;'
    if old_pos not in content:
        print("[WARN] cg_draw.c: DeathMessages x-Position nicht gefunden")
        return False
    content = content.replace(old_pos, new_pos, 1)

    upper_right_call = '\ty = CG_DrawDeathMessages( y );'
    if upper_right_call in content:
        content = content.replace(upper_right_call,
                                  '\t/* HHA_KILLFEED_LEFT: verschoben */', 1)

    hook_anchor = 'CG_HHA_Radar_Update();'
    if hook_anchor in content:
        content = content.replace(
            hook_anchor,
            '/* HHA_KILLFEED_LEFT */\n'
            '\t{\n\t\tfloat kfY = 24.0f;\n\t\tCG_DrawDeathMessages( kfY );\n\t}\n\n'
            '\t' + hook_anchor,
            1
        )
        print("[PATCHED] DeathMessages nach CG_Draw2D verschoben")
...
def patch_hud_compass():
    """v15.2: Kompass + Bedrohungs-Punkte. Enthaelt HHA_SHARED_BLOCK."""
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_draw.c" in files:
            target_file = os.path.join(root, "cg_draw.c")
            break
    if not target_file or not os.path.exists(target_file):
        print("[WARN] cg_draw.c not found (compass)")
        return False
    with open(target_file, "r", encoding="utf-8", errors="ignore") as f:
        content = f.read()

    if "HHA_COMPASS_V2" in content:
        print("[SKIP] Compass v15.2 bereits gepatcht")
        return True

    if "HHA_COMPASS_V1" in content:
        idx = content.find("HHA_COMPASS_V1")
        start = content.rfind("/* =====", 0, idx)
        if start >= 0:
            end_marker = "/* ============================================================ */"
            end = content.find(end_marker, idx)
            if end >= 0:
                ...
                content = content[:start] + content[end:]
                print("[REMOVED] alter HHA_COMPASS_V1 Block")

    compass_code = HHA_SHARED_BLOCK + r'''

/* ============================================================
 * [PATCHED v15.2] Kompass + Bedrohungs-Punkte
 * HHA_COMPASS_V2
 * ============================================================ */

#define HHA_COMPASS_STRIP_W   320.0f
#define HHA_COMPASS_STRIP_H    20.0f
#define HHA_COMPASS_VIEW_DEG   60.0f

static const char *hha_compass_names[8] = {
    "N", "NE", "E", "SE", "S", "SW", "W", "NW"
};
static const float hha_compass_angles[8] = {
    0.0f, 45.0f, 90.0f, 135.0f, 180.0f, 225.0f, 270.0f, 315.0f
};

void CG_HandheldDrawCompass( void ) {
    char  buf[8];
    int   i;
    float yaw, cx, cy, halfW;
    float colActive[4]  = { 1.00f, 0.95f, 0.40f, 0.95f };
    float colNormal[4]  = { 0.85f, 0.85f, 0.85f, 0.70f };
    float colBg[4]      = { 0.00f, 0.00f, 0.00f, 0.50f };
    float colBorder[4]  = { 0.55f, 0.55f, 0.55f, 0.55f };

    if ( !cg.snap ) return;
    trap_Cvar_VariableStringBuffer("cg_handheldCompass", buf, sizeof(buf));
    if ( atof(buf) == 0.0f ) return;
    ...
    cx = 320.0f;
    cy = 4.0f;
    halfW = HHA_COMPASS_STRIP_W * 0.5f;

    trap_R_SetColor( colBg );
    CG_DrawPic( cx - halfW, cy, HHA_COMPASS_STRIP_W, HHA_COMPASS_STRIP_H,
                cgs.media.whiteShader );
    trap_R_SetColor( colBorder );
    CG_DrawPic( cx - halfW, cy, HHA_COMPASS_STRIP_W, 1.0f, cgs.media.whiteShader );
    CG_DrawPic( cx - halfW, cy + HHA_COMPASS_STRIP_H - 1.0f, HHA_COMPASS_STRIP_W, 1.0f,
                cgs.media.whiteShader );
    CG_DrawPic( cx - 0.5f, cy + 1.0f, 1.0f, 6.0f, cgs.media.whiteShader );
    trap_R_SetColor( NULL );

    for ( i = 0; i < 8; i++ ) {
        float diff = hha_compass_angles[i] - yaw;
        float xpos, labelW;
        float *col;

        while ( diff > 180.0f ) diff -= 360.0f;
        while ( diff < -180.0f ) diff += 360.0f;
        if ( diff < -HHA_COMPASS_VIEW_DEG * 0.5f ) continue;
        if ( diff >  HHA_COMPASS_VIEW_DEG * 0.5f ) continue;

        xpos = cx + ( diff / ( HHA_COMPASS_VIEW_DEG * 0.5f ) ) * halfW;
        col = ( i == 0 ) ? colActive : colNormal;
        col[3] *= 1.0f - ( fabsf( diff ) / ( HHA_COMPASS_VIEW_DEG * 0.5f ) ) * 0.3f;

        labelW = CG_Text_Width( hha_compass_names[i], 0.28f, 0 );
        CG_Text_Paint( xpos - labelW * 0.5f, cy + 12.0f, 0.28f, col,
                       hha_compass_names[i], 0, 0, ITEM_TEXTSTYLE_SHADOWED );

        trap_R_SetColor( colNormal );
        CG_DrawPic( xpos - 0.5f, cy + 13.0f, 1.0f, 2.0f, cgs.media.whiteShader );
        trap_R_SetColor( NULL );
    }

    for ( i = 0; i < hha_threatNum; i++ ) {
        hha_threat_t *t = &hha_threatCache[i];
        float diff = t->bearing - yaw;
        float xpos, dotSize, alpha;
        float dotCol[4];
        ...
        if ( t->hasLoS ) {
            dotCol[0] = 1.00f; dotCol[1] = 0.20f; dotCol[2] = 0.10f;
        } else if ( t->soundStrength > 0.4f ) {
            dotCol[0] = 1.00f; dotCol[1] = 0.65f; dotCol[2] = 0.10f;
        } else if ( t->soundStrength > 0.05f || t->isLooping ) {
            dotCol[0] = 0.85f; dotCol[1] = 0.85f; dotCol[2] = 0.20f;
        } else {
            dotCol[0] = 0.55f; dotCol[1] = 0.55f; dotCol[2] = 0.55f;
        }
        dotCol[3] = alpha;

        dotSize = 4.0f + t->soundStrength * 2.0f;
        if ( dotSize > 8.0f ) dotSize = 8.0f;

        trap_R_SetColor( dotCol );
        CG_DrawPic( xpos - dotSize * 0.5f,
                    cy + HHA_COMPASS_STRIP_H - dotSize - 1.0f,
                    dotSize, dotSize, cgs.media.whiteShader );
        trap_R_SetColor( NULL );
    }
}
...
'''
    ...
    if not inc_pat.search(content):
        return False
    content = inc_pat.sub(lambda m: m.group(1) + compass_code, content, count=1)

    hook_pat = re.compile(r'(\n[ \t]*CG_HHA_Radar_Update\s*\(\s*\)\s*;)')
    if hook_pat.search(content):
        content = hook_pat.sub(
            r'\1\n\tCG_HandheldDrawCompass();',
            content, count=1
        )
        print("[PATCHED] Compass v15.2 Hook")
...
def patch_hud_low_health_vignette():
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_draw.c" in files:
            target_file = os.path.join(root, "cg_draw.c")
            break
    if not target_file or not os.path.exists(target_file):
        print("[WARN] cg_draw.c not found (vignette)")
        return False
    with open(target_file, "r", encoding="utf-8", errors="ignore") as f:
        content = f.read()

    if "HHA_LOWHEALTH_V1" in content:
        print("[SKIP] Low-Health-Vignette bereits gepatcht")
        return True

    vignette_code = r'''

/* [PATCHED v15.1] Low-Health-Vignette HHA_LOWHEALTH_V1 */
void CG_HandheldDrawLowHealthVignette( void ) {
    int    hp;
    float  alpha, pulse;
    float  color[4];
    char   buf[8];
    ...
    if ( !cg.snap ) return;
    trap_Cvar_VariableStringBuffer("cg_handheldLowHealthVignette", buf, sizeof(buf));
    if ( atof(buf) == 0.0f ) return;
    if ( cg.snap->ps.stats[STAT_HEALTH] <= 0 ) return;
    if ( cg.renderingThirdPerson ) return;
    ...
    alpha = 1.0f - ( (float)hp / 35.0f );
    if ( alpha < 0.0f ) alpha = 0.0f;
    if ( alpha > 1.0f ) alpha = 1.0f;

    pulse = 1.0f;
    if ( hp < 15 ) pulse = 0.6f + 0.4f * sinf( (float)cg.time * 0.008f );

    alpha *= 0.55f * pulse;

    color[0] = 0.7f; color[1] = 0.0f; color[2] = 0.0f; color[3] = alpha;
    trap_R_SetColor( color );
    CG_DrawPic( 0, 0, 640, 24, cgs.media.whiteShader );
    CG_DrawPic( 0, 456, 640, 24, cgs.media.whiteShader );
    CG_DrawPic( 0, 0, 20, 480, cgs.media.whiteShader );
    CG_DrawPic( 620, 0, 20, 480, cgs.media.whiteShader );
    trap_R_SetColor( NULL );
}
/* ============================================================ */
'''
    ...
    if not inc_pat.search(content):
        return False
    content = inc_pat.sub(lambda m: m.group(1) + vignette_code, content, count=1)

    hook_anchor = 'CG_HHA_Radar_Update();'
    if hook_anchor in content:
        content = content.replace(
            hook_anchor,
            'CG_HandheldDrawLowHealthVignette();\n\t' + hook_anchor,
            1
        )
        print("[PATCHED] Low-Health-Vignette v15.1")
...
def patch_hud_mini_radar():
    """v15.5: Mini-Radar. Enthaelt HHA_SHARED_BLOCK."""
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_draw.c" in files:
            target_file = os.path.join(root, "cg_draw.c")
            break
    if not target_file or not os.path.exists(target_file):
        print("[WARN] cg_draw.c not found (mini radar)")
        return False
    with open(target_file, "r", encoding="utf-8", errors="ignore") as f:
        content = f.read()

    if "HHA_MINIRADAR_V2" in content:
        print("[SKIP] Mini-Radar v15.5 bereits gepatcht")
        return True

    if "HHA_MINIRADAR_V1" in content:
        idx = content.find("HHA_MINIRADAR_V1")
        start = content.rfind("/* =====", 0, idx)
        if start >= 0:
            end_marker = "/* ============================================================ */"
            end = content.find(end_marker, idx)
            if end >= 0:
                ...
                content = content[:start] + content[end:]
                print("[REMOVED] alter HHA_MINIRADAR_V1 Block")

    mini_radar_code = HHA_SHARED_BLOCK + r'''

/* ============================================================
 * [PATCHED v15.5] Mini-Radar mit Priority + Damage + Off-Screen
 * HHA_MINIRADAR_V2
 * ============================================================ */

#define HHA_MINIRADAR_CX          576.0f
#define HHA_MINIRADAR_CY           90.0f
#define HHA_MINIRADAR_RADIUS       46.0f
#define HHA_MINIRADAR_RING_DOTS    64
#define HHA_MINIRADAR_BORDER_DOTS 128

static float HHA_MiniRadar_Priority( hha_threat_t *t, float maxRange ) {
    float p = 0.0f;
    float distFactor = 1.0f - ( t->distance / maxRange );
    if ( distFactor < 0.0f ) distFactor = 0.0f;
    if ( t->hasLoS ) p += 3.0f;
    p += t->soundStrength * 2.0f;
    p += distFactor * 1.5f;
    return p;
}

static float HHA_MiniRadar_DamageBearing( void ) {
    float viewYaw, damageViewAngle, bearing;
    if ( cg.damageTime <= 0.0f ) return -1.0f;
    viewYaw = cg.refdefViewAngles[YAW];
    while ( viewYaw < 0.0f ) viewYaw += 360.0f;
    while ( viewYaw >= 360.0f ) viewYaw -= 360.0f;
    damageViewAngle = atan2f( cg.damageY, cg.damageX ) * ( 180.0f / M_PI );
    bearing = viewYaw - damageViewAngle;
    while ( bearing < 0.0f ) bearing += 360.0f;
    while ( bearing >= 360.0f ) bearing -= 360.0f;
    return bearing;
}

void CG_HHA_DrawMiniRadar( void ) {
    char  buf[16];
    int   mode, i, j;
    float maxRange;
    float colBg[4], colRing[4], colBorder[4], colPlayer[4], colNorth[4];
    float cx, cy, r_out;
    float playerYaw;
    int   bestIdx = -1;
    ...
    trap_Cvar_VariableStringBuffer("cg_handheldMiniRadar", buf, sizeof(buf));
    if ( atof(buf) == 0.0f ) return;
    ...
    cx = HHA_MINIRADAR_CX;
    cy = HHA_MINIRADAR_CY;
    r_out = HHA_MINIRADAR_RADIUS;

    maxRange = HHA_Threat_CvarFloat("cg_handheldRadarRange");
    if ( maxRange < 500.0f ) maxRange = 500.0f;
    if ( maxRange > 8000.0f ) maxRange = 8000.0f;

    for ( i = 0; i < hha_threatNum; i++ ) {
        float p = HHA_MiniRadar_Priority( &hha_threatCache[i], maxRange );
        if ( p > bestPriority ) { bestPriority = p; bestIdx = i; }
    }

    colBg[0] = 0.0f; colBg[1] = 0.0f; colBg[2] = 0.0f; colBg[3] = 0.60f;
    trap_R_SetColor( colBg );
    CG_DrawPic( cx - r_out, cy - r_out, r_out * 2.0f, r_out * 2.0f,
                cgs.media.whiteShader );
    trap_R_SetColor( NULL );

    colRing[0] = 0.5f; colRing[1] = 0.5f; colRing[2] = 0.5f; colRing[3] = 0.35f;
    trap_R_SetColor( colRing );
    for ( i = 1; i <= 3; i++ ) {
        float ringR = r_out * ( (float)i / 3.0f );
        for ( j = 0; j < HHA_MINIRADAR_RING_DOTS; j++ ) {
            float a = (float)j * ( 2.0f * M_PI / (float)HHA_MINIRADAR_RING_DOTS );
            CG_DrawPic( cx + sinf(a)*ringR - 0.5f, cy - cosf(a)*ringR - 0.5f,
                        1.0f, 1.0f, cgs.media.whiteShader );
        }
    }
    trap_R_SetColor( NULL );

    if ( cg.damageTime > 0.0f ) {
        int elapsed = cg.time - (int)cg.damageTime;
        if ( elapsed < 0 ) elapsed += 1000;
        if ( elapsed >= 0 && elapsed < 900 ) {
            float alpha = 1.0f - ( (float)elapsed / 900.0f );
            float pulse = 0.6f + 0.4f * sinf( (float)cg.time * 0.03f );
            float bearing = HHA_MiniRadar_DamageBearing();
            if ( bearing >= 0.0f ) {
                float relDamage = bearing - cg.refdefViewAngles[YAW];
                while ( relDamage > 180.0f ) relDamage -= 360.0f;
                while ( relDamage < -180.0f ) relDamage += 360.0f;
                float a = relDamage * ( M_PI / 180.0f );
                float colDmg[4];
                colDmg[0] = 1.0f; colDmg[1] = 0.10f; colDmg[2] = 0.05f;
                colDmg[3] = alpha * pulse;
                trap_R_SetColor( colDmg );
                for ( i = -15; i <= 15; i++ ) {
                    float aa = a + ( (float)i * ( M_PI / 180.0f ) );
                    float px = cx + sinf(aa) * r_out;
                    float py = cy - cosf(aa) * r_out;
                    CG_DrawPic( px - 1.5f, py - 1.5f, 3.0f, 3.0f, cgs.media.whiteShader );
                }
                trap_R_SetColor( NULL );
            }
        }
    ...
    colBorder[0] = 0.75f; colBorder[1] = 0.75f; colBorder[2] = 0.75f; colBorder[3] = 0.70f;
    trap_R_SetColor( colBorder );
    for ( i = 0; i < HHA_MINIRADAR_BORDER_DOTS; i++ ) {
        float a = (float)i * ( 2.0f * M_PI / (float)HHA_MINIRADAR_BORDER_DOTS );
        CG_DrawPic( cx + sinf(a)*r_out - 0.5f, cy - cosf(a)*r_out - 0.5f,
                    1.0f, 1.0f, cgs.media.whiteShader );
    }
    trap_R_SetColor( NULL );

    colPlayer[0] = 0.90f; colPlayer[1] = 0.90f; colPlayer[2] = 0.30f; colPlayer[3] = 1.0f;
    trap_R_SetColor( colPlayer );
    CG_DrawPic( cx - 3.0f, cy - 3.0f, 6.0f, 6.0f, cgs.media.whiteShader );
    CG_DrawPic( cx - 1.0f, cy - 8.0f, 2.0f, 4.0f, cgs.media.whiteShader );
    trap_R_SetColor( NULL );
    ...
    while ( playerYaw >= 360.0f ) playerYaw -= 360.0f;
    {
        float northAngle = ( -playerYaw ) * ( M_PI / 180.0f );
        float nx = cx + sinf( northAngle ) * ( r_out + 3.0f );
        float ny = cy - cosf( northAngle ) * ( r_out + 3.0f );
        colNorth[0] = 1.00f; colNorth[1] = 0.95f; colNorth[2] = 0.40f; colNorth[3] = 0.95f;
        trap_R_SetColor( colNorth );
        CG_DrawPic( nx - 3.0f, ny - 3.0f, 6.0f, 6.0f, cgs.media.whiteShader );
        trap_R_SetColor( NULL );
    }

    for ( i = 0; i < hha_threatNum; i++ ) {
        hha_threat_t *t = &hha_threatCache[i];
        float angleRad, dotX, dotY, scaledDist, dotSize;
        float col[4];
        qboolean isPriority = ( i == bestIdx );
        qboolean offScreen = ( t->distance > maxRange );

        angleRad = t->relAngle * ( M_PI / 180.0f );
        scaledDist = t->distance / maxRange;
        if ( scaledDist > 1.0f ) scaledDist = 1.0f;
        if ( scaledDist < 0.0f ) scaledDist = 0.0f;

        if ( t->hasLoS ) {
            col[0] = 1.00f; col[1] = 0.20f; col[2] = 0.10f;
        } else if ( t->soundStrength > 0.4f ) {
            col[0] = 1.00f; col[1] = 0.65f; col[2] = 0.10f;
        } else if ( t->soundStrength > 0.05f || t->isLooping ) {
            col[0] = 0.85f; col[1] = 0.85f; col[2] = 0.20f;
        } else {
            col[0] = 0.55f; col[1] = 0.55f; col[2] = 0.55f;
        }
        col[3] = 0.75f + t->soundStrength * 0.25f;
        if ( col[3] > 1.0f ) col[3] = 1.0f;

        if ( offScreen ) {
            float ax = cx + sinf( angleRad ) * ( r_out + 5.0f );
            float ay = cy - cosf( angleRad ) * ( r_out + 5.0f );
            trap_R_SetColor( col );
            CG_DrawPic( ax - 3.0f, ay - 3.0f, 6.0f, 6.0f, cgs.media.whiteShader );
            {
                float tipX = cx + sinf( angleRad ) * ( r_out + 10.0f );
                float tipY = cy - cosf( angleRad ) * ( r_out + 10.0f );
                CG_DrawPic( tipX - 2.0f, tipY - 2.0f, 4.0f, 4.0f, cgs.media.whiteShader );
            }
            trap_R_SetColor( NULL );
            if ( isPriority ) {
                float ringCol[4] = { 1.0f, 1.0f, 1.0f, 0.85f };
                trap_R_SetColor( ringCol );
                CG_DrawPic( ax - 5.0f, ay - 5.0f, 10.0f, 10.0f, cgs.media.whiteShader );
                trap_R_SetColor( NULL );
            }
            continue;
        }

        dotX = cx + sinf( angleRad ) * scaledDist * r_out;
        dotY = cy - cosf( angleRad ) * scaledDist * r_out;

        dotSize = 5.0f + ( 1.0f - scaledDist ) * 4.0f;
        if ( dotSize < 5.0f ) dotSize = 5.0f;
        if ( dotSize > 9.0f ) dotSize = 9.0f;

        if ( isPriority ) {
            float ringCol[4] = { 1.0f, 1.0f, 1.0f, 0.85f };
            trap_R_SetColor( ringCol );
            CG_DrawPic( dotX - dotSize * 0.5f - 2.0f,
                        dotY - dotSize * 0.5f - 2.0f,
                        dotSize + 4.0f, dotSize + 4.0f,
                        cgs.media.whiteShader );
            trap_R_SetColor( NULL );
        }
        ...
        if ( t->soundStrength > 0.7f ) {
            float pulse = 0.6f + 0.4f * sinf( (float)cg.time * 0.02f );
            col[3] *= pulse;
            trap_R_SetColor( col );
            CG_DrawPic( dotX - dotSize * 0.5f - 3.0f,
                        dotY - dotSize * 0.5f - 3.0f,
                        dotSize + 6.0f, dotSize + 6.0f,
                        cgs.media.whiteShader );
            trap_R_SetColor( NULL );
        }
    }
    ...
    if ( cg.damageTime > 0.0f ) {
        ...
        if ( elapsed >= 0 && elapsed < 900 ) {
            ...
            if ( bearing >= 0.0f ) {
                ...
                while ( relDamage < -180.0f ) relDamage += 360.0f;
                float a = relDamage * ( M_PI / 180.0f );
                float colSpike[4] = { 1.0f, 0.15f, 0.05f, alpha };
                int seg;
                trap_R_SetColor( colSpike );
                for ( seg = 0; seg < 12; seg++ ) {
                    float r1 = r_out * ( (float)seg / 12.0f );
                    float px = cx + sinf(a) * r1;
                    float py = cy - cosf(a) * r1;
                    CG_DrawPic( px - 1.0f, py - 1.0f, 2.0f, 2.0f,
                                cgs.media.whiteShader );
                }
                trap_R_SetColor( NULL );
            }
...
'''
    ...
    inc_pat = re.compile(r'(#include\s+"cg_local\.h"\s*\n)')
    if not inc_pat.search(content):
        print("[WARN] cg_draw.c: cg_local.h include nicht gefunden (miniradar)")
        return False
    content = inc_pat.sub(lambda m: m.group(1) + mini_radar_code, content, count=1)

    hook_pat = re.compile(
        r'(\n[ \t]*CG_HHA_DrawEdgeIndicators\s*\(\s*\)\s*;)'
    )
    if hook_pat.search(content):
        content = hook_pat.sub(
            r'\1\n\tCG_HHA_DrawMiniRadar();',
            content, count=1
        )
        print("[PATCHED] Mini-Radar v15.5 Hook")
    else:
        print("[WARN] Mini-Radar: CG_HHA_DrawEdgeIndicators anchor fehlt")
...
def patch_vm_pure_gate():
    ...
    if touched == 0:
        print("[INFO] VM pure-gate nicht gefunden")
    return touched > 0
...
def patch_vm_force_native():
    ...
    with open(vm_c, "w", encoding="utf-8") as f:
        f.write(content)
    print("[PATCHED] vm.c: enableDll=1 erzwungen")
    return True
...
itemDef { name window group grpAimButton rect 2 2 632 367 style WINDOW_STYLE_FILLED border 1 bordercolor .5 .5 .5 .5 forecolor 1 1 1 1 backcolor 0 0 0 .5 visible 1 decoration }
...
itemDef { name aim_hint_preset style 1 text "--- PRESET WAEHLEN (Links/Rechts) ---" rect 20 36 600 14 textalign ITEM_ALIGN_CENTER textalignx 300 textaligny 12 textscale .22 forecolor .7 .7 .7 1 visible 1 decoration }
itemDef { name preset_select group grpAim type ITEM_TYPE_MULTI text "Preset:" cvar "cg_handheldAimPreset" cvarFloatList { "0 - CUSTOM" 0 "1 - ONLINE (Snap-On)" 1 "2 - OFFLINE (Bots)" 2 "3 - AIMBOT (MAX)" 3 } rect 100 56 440 24 textalign ITEM_ALIGN_RIGHT textalignx 220 textaligny 16 textscale .30 forecolor 1 .75 0 1 visible 1 }
itemDef { name aim_hint_qol style 1 text "--- LIVE WERTE ---" rect 180 90 280 12 textalign ITEM_ALIGN_CENTER textalignx 140 textaligny 10 textscale .20 forecolor .7 .7 .7 1 visible 1 decoration }
itemDef { name aim_options group grpAim type ITEM_TYPE_YESNO text "Handheld Aim Assist:" cvar "cg_handheldAimAssist" rect 60 108 240 18 textalign ITEM_ALIGN_RIGHT textalignx 160 textaligny 15 textscale .28 forecolor 1 1 1 1 visible 1 }
itemDef { name aim_options group grpAim type ITEM_TYPE_YESNO text "HardLock (Snap-On):" cvar "cg_handheldAimAssistHardLock" rect 60 130 240 18 textalign ITEM_ALIGN_RIGHT textalignx 160 textaligny 15 textscale .28 forecolor 1 .3 .3 1 visible 1 }
itemDef { name aim_options group grpAim type ITEM_TYPE_YESNO text "Fire Assist:" cvar "cg_handheldAimAssistFire" rect 60 152 240 18 textalign ITEM_ALIGN_RIGHT textalignx 160 textaligny 15 textscale .28 forecolor 1 1 1 1 visible 1 }
itemDef { name aim_options group grpAim type ITEM_TYPE_SLIDER text "Aim Cone Angle:" cvarfloat "cg_handheldAimAssistAngle" 89 5 89 rect 60 174 240 18 textalign ITEM_ALIGN_RIGHT textalignx 160 textaligny 12 textscale .28 forecolor 1 1 1 1 visible 1 }
itemDef { name aim_options group grpAim type ITEM_TYPE_SLIDER text "Magnetism Strength:" cvarfloat "cg_handheldAimAssistStrength" 1.20 0.05 1.20 rect 60 196 240 18 textalign ITEM_ALIGN_RIGHT textalignx 160 textaligny 12 textscale .28 forecolor 1 1 1 1 visible 1 }
itemDef { name aim_options group grpAim type ITEM_TYPE_SLIDER text "Input Friction:" cvarfloat "cg_handheldAimAssistFriction" 0.50 0.50 1.00 rect 60 218 240 18 textalign ITEM_ALIGN_RIGHT textalignx 160 textaligny 12 textscale .28 forecolor 1 1 1 1 visible 1 }
itemDef { name aim_options group grpAim type ITEM_TYPE_YESNO text "Auto Weapon Switch:" cvar "cg_handheldAutoSwitch" rect 60 244 240 18 textalign ITEM_ALIGN_RIGHT textalignx 160 textaligny 15 textscale .28 forecolor 1 1 1 1 visible 1 }
itemDef { name aim_options group grpAim type ITEM_TYPE_YESNO text "Hit Marker:" cvar "cg_handheldHitMarker" rect 60 266 240 18 textalign ITEM_ALIGN_RIGHT textalignx 160 textaligny 15 textscale .28 forecolor 1 1 1 1 visible 1 }
itemDef { name aim_options group grpAim type ITEM_TYPE_YESNO text "Damage Indicator:" cvar "cg_handheldDamageIndicator" rect 60 288 240 18 textalign ITEM_ALIGN_RIGHT textalignx 160 textaligny 15 textscale .28 forecolor 1 1 1 1 visible 1 }
itemDef { name aim_options group grpAim type ITEM_TYPE_YESNO text "Enemy Outline:" cvar "cg_handheldEnemyOutline" rect 60 310 240 18 textalign ITEM_ALIGN_RIGHT textalignx 160 textaligny 15 textscale .28 forecolor 1 1 1 1 visible 1 }
itemDef { name nav1 group grpAim type ITEM_TYPE_BUTTON text "Advanced..." rect 20 342 190 24 textalign ITEM_ALIGN_CENTER textalignx 95 textaligny 17 textscale .26 forecolor 1 .75 0 1 visible 1 action { play "sound/misc/menu3.wav" ; close aim_assist_menu ; open aim_assist_advanced_menu } }
itemDef { name nav2 group grpAim type ITEM_TYPE_BUTTON text "Dynamic Zoom..." rect 225 342 190 24 textalign ITEM_ALIGN_CENTER textalignx 95 textaligny 17 textscale .26 forecolor 1 .75 0 1 visible 1 action { play "sound/misc/menu3.wav" ; close aim_assist_menu ; open aim_assist_zoom_menu } }
itemDef { name nav3 group grpAim type ITEM_TYPE_BUTTON text "Zurueck" rect 430 342 190 24 textalign ITEM_ALIGN_CENTER textalignx 95 textaligny 17 textscale .26 forecolor 1 1 1 1 visible 1 action { play "sound/misc/menu3.wav" ; close aim_assist_menu ; open options_menu } }
}
}
"""
    with open(os.path.join(menu_dir, "settings_aimassist.menu"),
        ...
        f.write(aim_menu)
    print("[PATCHED] settings_aimassist.menu")
...
qboolean FS_NativeFindFile( const char *basename,
    ...
    if ( !basename || !*basename || !outPath || outSize <= 0 ) return qfalse;
...
def main():
    ...
    applied = 0
    if is_smokinguns:
        print("==> Patching SmokinGuns (v15.5-fix2)")
        diagnostic_dump()
        ...
        # --- Client-VM (cl_main.c, cl_input.c, cl_console.c) ---
        patch_client_cvar()
        patch_aim_assist()
        patch_cl_main_namerotator()
        patch_cl_console_chatprefill()

        # --- UI-VM (ui_main.c, ui_local.h) ---
        patch_ui_cvar()

        # --- cgame-VM: andere Dateien ---
        patch_cg_view()
        patch_cg_ents_sound_hook()
        patch_cg_event_sound_hook()
        patch_cg_event_hit()
        patch_cg_servercmds_rename_filter()
        patch_hud_killfeed_filter()
        patch_hud_obituary_filter()
        patch_hud_hitmessage_filter()

        # --- cg_draw.c Patches ---
        # Jeder Patch fuegt seinen Code direkt nach #include "cg_local.h" ein.
        # Der zuletzt aufgerufene Patch landet also ganz oben in der Datei.
        # Damit hha_threat_t in jedem Fall vor der ersten Verwendung steht,
        # enthaelt jeder der drei Patches (radar, mini_radar, compass) den
        # HHA_SHARED_BLOCK mit einem Preprocessor-Guard. Nur der erste
        # Block wird tatsaechlich kompiliert.
        patch_hud_compass()
        patch_hud_mini_radar()
        patch_hud_low_health_vignette()
        patch_hud_killfeed_move_left()
        patch_cg_hitmarker()
        patch_cg_enemy_outline()
        patch_cg_crosshair_player()
        patch_cg_crosshair_min_size()
        patch_cg_hha_radar()
