...
def patch_cg_crosshair_min_size():
    ...
    if n == 0:
        return False
    print("[PATCHED] Crosshair Mindestgroesse 25 in cg_draw.c")
...
def patch_cg_hha_radar():
    """v15.3: Threat-Daten + Edge-Indicators."""
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_draw.c" in files:
            target_file = os.path.join(root, "cg_draw.c")
    ...
    # Alten HHA_RADAR_V1 Block entfernen
...
def patch_hud_compass():
    """v15.2: Kompass oben Mitte mit integrierten Bedrohungs-Punkten."""
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_draw.c" in files:
            target_file = os.path.join(root, "cg_draw.c")
...
def patch_hud_mini_radar():
    """v15.5: Mini-Radar mit Priority + Damage + Off-Screen-Arrows."""
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_draw.c" in files:
            target_file = os.path.join(root, "cg_draw.c")
...
void CG_HHA_DrawMiniRadar( void ) {
    ...
    /* On-Screen Marker + Off-Screen Arrows */
    for ( i = 0; i < hha_threatNum; i++ ) {
        hha_threat_t *t = &hha_threatCache[i];
        ...
        if ( offScreen ) {
            /* Arrow am Radar-Rand, zeigt nach aussen */
            float ax, ay;
            ax = cx + sinf( angleRad ) * ( r_out + 5.0f );
            ay = cy - cosf( angleRad ) * ( r_out + 5.0f );

            /* Kleines Dreieck: Basis 6px, Spitze 4px nach aussen */
            trap_R_SetColor( col );
            /* Basis-Quadrat */
            CG_DrawPic( ax - 3.0f, ay - 3.0f, 6.0f, 6.0f, cgs.media.whiteShader );
            /* Spitze als duenner Streifen nach aussen */
            {
            ...
            /* Priority-Ring auch bei off-screen */
            if ( isPriority ) {
    ...
    /* Damage-Spike vom Zentrum zur Aussenkante */
    if ( cg.damageTime > 0.0f ) {
        int elapsed = cg.time - (int)cg.damageTime;
        if ( elapsed < 0 ) elapsed += 1000;
        if ( elapsed >= 0 && elapsed < 900 ) {
...
def main():
    ...
    applied = 0
    if is_smokinguns:
        print("==> Patching SmokinGuns (v15.5)")
        diagnostic_dump()
