...
v13.7 - LUT-Fix + Aimbot-Ballistik + Iterative Vorhersage.
"""
...
def patch_aim_assist():
    ...
    helper_code = r"""
/* ============================================================
 * [PATCHED v13.7] Handheld Aim Assist (Client-Seite)
 * v13.7: LUT-Fix + Aimbot-Ballistik + Iterative Vorhersage.
 * ============================================================ */
#include <math.h>
...
static int hha_lastMagnetismTime = 0;

/* [PATCH v13.7] Curve-Lookup-Table — MUSS vor CL_HandheldInputCurve
 * deklariert sein, sonst "undeclared identifier" beim Kompilieren. */
static float     hha_curveLUT[128];
static qboolean  hha_curveLUTReady = qfalse;

static void HHA_InitCurveLUT( void ) {
    int i;
    for ( i = 0; i < 128; i++ ) {
        float a = (float)i;
        hha_curveLUT[i] = powf( a, HHA_CURVE_EXP ) /
                          powf( HHA_CURVE_REF, HHA_CURVE_EXP - 1.0f );
    }
    hha_curveLUTReady = qtrue;
}
...
static void HHA_ReadConfig( hha_config_t *cfg ) {
    ...
    /* [PATCHED v13.6] Vorberechnete Cosinus-Schwelle */
    {
        float rad = cfg->angle * ( HHA_PI / 180.0f );
        cfg->cosThreshold = cosf( rad );
        if ( cfg->cosThreshold >  1.0f ) cfg->cosThreshold =  1.0f;
        if ( cfg->cosThreshold < -1.0f ) cfg->cosThreshold = -1.0f;
    }
}
...
/* [PATCH v13.7] Aimbot-Ballistik: Iterative, geschwindigkeits-skalierte
 * Vorhersage. Alle Feuerwaffen in Smokin' Guns sind Hitscan (EV_BULLET),
 * d.h. Projektilgeschwindigkeit = unendlich. Die relevante Größe ist
 * daher Netzwerklatenz + Frame-Delay, NICHT Waffengeschwindigkeit.
 * Die Iteration verfeinert die Vorhersage distanzabhängig. */
static void HHA_GetPredictedPos_Aimbot( const entityState_t *ent,
                                        const hha_config_t *cfg,
                                        vec3_t out ) {
    vec3_t targetVel;
    float  targetSpeed;
    float  leadTime;
    ...
    /* Ziel-Geschwindigkeit (units/sec) */
    VectorCopy( ent->pos.trDelta, targetVel );
    targetSpeed = sqrtf( targetVel[0]*targetVel[0] +
                         targetVel[1]*targetVel[1] +
                         targetVel[2]*targetVel[2] );

    /* Basis-Lead: Alter des letzten Snapshots (Server→Client-Latenz) */
    snapAge = (float)( cl.serverTime - cl.snap.serverTime ) * 0.001f;
    if ( snapAge < 0.0f )    snapAge = 0.0f;
    if ( snapAge > 0.150f )  snapAge = 0.150f;
    leadTime = snapAge;

    /* Halbe-Ping-Kompensation (Unlagged-Prinzip) */
    if ( cl.snap.ping > 0 ) {
        float pingLead = (float)cl.snap.ping * 0.0005f;
        if ( pingLead > 0.150f ) pingLead = 0.150f;
        leadTime += pingLead;
    }

    /* Zusätzlicher Lead: skaliert mit Zielgeschwindigkeit.
     * Schnell bewegende Ziele (strafing) brauchen mehr Vorhalt,
     * um die 1-2 Frames Verzögerung bis zur Server-Verarbeitung
     * zu kompensieren. */
    if ( targetSpeed > 30.0f ) {
        leadTime += 0.020f + ( targetSpeed * 0.00008f );
    }
    /* Manueller Lead (Slider im Advanced-Menü) obendrauf */
    if ( cfg->leadTime > 0.0f ) leadTime += cfg->leadTime;
    if ( leadTime > 0.250f ) leadTime = 0.250f;

    /* Iterative Verfeinerung: Distanz-abhängige Skalierung.
     * Nahe Ziele: weniger Lead (Ziel ist bereits "groß" genug).
     * Ferne Ziele: mehr Lead (Winkelgeschwindigkeit täuscht). */
    VectorCopy( ent->pos.trBase, out );
    for ( iter = 0; iter < 3; iter++ ) {
        VectorSubtract( out, cl.snap.ps.origin, delta );
        distToTarget = sqrtf( delta[0]*delta[0] +
                              delta[1]*delta[1] +
                              delta[2]*delta[2] );

        if ( distToTarget < 200.0f ) {
            leadTime *= 0.85f;
        } else if ( distToTarget > 1500.0f ) {
            leadTime *= 1.05f;
        }

        out[0] = ent->pos.trBase[0] + targetVel[0] * leadTime;
        out[1] = ent->pos.trBase[1] + targetVel[1] * leadTime;
        out[2] = ent->pos.trBase[2] + targetVel[2] * leadTime;
    }
}

static void HHA_GetPredictedPos( const entityState_t *ent, const hha_config_t *cfg, vec3_t out ) {
    /* [PATCH v13.7] Im Aimbot-Modus (HardLock) die präzise,
     * iterative Vorhersage verwenden. */
    if ( cfg->hardLock ) {
        HHA_GetPredictedPos_Aimbot( ent, cfg, out );
        return;
    }

    /* Standard-Modus: lineare Extrapolation mit Latenz-Kompensation */
    float dt = 0.0f;
    {
...
static int HHA_FindTarget( const hha_config_t *cfg, vec3_t bestDir,
                           float *bestAngleOut ) {
    ...
    *bestAngleOut = 999.0f;
    ...
    for ( i = 0; i < cl.snap.numEntities && ncands < HHA_MAX_CANDIDATES; i++ ) {
        ...
        /* [PATCHED v13.6] Prefilter per Cosinus-Schwelle — kein acosf */
        if ( dot < cfg->cosThreshold ) continue;
        if ( dot != dot ) continue;   /* NaN-Check */

        /* Sticky-Bonus: Wenn der letzte Target in erweitertem
         * Winkelbereich liegt, bevorzuge ihn durch künstlichen
         * dot-Boost (entspricht angle *= 0.60). */
        if ( ent->number == hha_lastTargetNum ) {
            float stickyDot = cosf( cfg->angle * 0.85f *
                                    ( HHA_PI / 180.0f ) );
            if ( dot > stickyDot ) {
                dot = ( dot + 1.0f ) * 0.5f;
            }
    ...
    /* Sortierung: dot ABSTEIGEND (größerer dot = kleinerer Winkel) */
    for ( i = 0; i < ncands; i++ ) {
        for ( j = i + 1; j < ncands; j++ ) {
            if ( cands[j].dot > cands[i].dot ) {
    ...
    for ( i = 0; i < ncands; i++ ) {
        ...
        if ( ok ) {
            /* acosf NUR für den Gewinner */
            float angle = acosf( cands[i].dot ) * ( 180.0f / HHA_PI );
...
static void CL_HandheldInputCurve( void ) {
    ...
    int   *mx_raw, *my_raw;
    float  friction = 1.0f;
    static float mx_remainder = 0.0f;
    static float my_remainder = 0.0f;

    if ( !hha_configValid ) { HHA_ReadConfig( cfg ); hha_configValid = qtrue; }
    if ( !cfg->enabled ) return;

    if ( !hha_curveLUTReady ) HHA_InitCurveLUT();

    mx_raw = &cl.mouseDx[cl.mouseIndex];
    my_raw = &cl.mouseDy[cl.mouseIndex];

    {
        float mag = sqrtf( (float)(*mx_raw * *mx_raw) +
                           (float)(*my_raw * *my_raw) );
    ...
    if ( *mx_raw != 0 ) {
        ...
        float curved;
        if ( a > 127 ) a = 127;
        curved = hha_curveLUT[a] * friction * s;
        curved += mx_remainder;
        *mx_raw = (int)curved;
        mx_remainder = curved - (float)( *mx_raw );
    } else {
        mx_remainder = 0.0f;
    }
    ...
    if ( *my_raw != 0 ) {
        ...
        curved += my_remainder;
        *my_raw = (int)curved;
        my_remainder = curved - (float)( *my_raw );
    } else {
        my_remainder = 0.0f;
    }
}
...
static void CL_HandheldAimMagnetism( usercmd_t *cmd ) {
    ...
    /* [PATCHED v13.6] Framerate-unabhängiges Exponential-Smoothing.
     * pullYaw/pullPitch sind Blend-Faktoren (0..cap), die bei 60 FPS
     * als Rate interpretiert werden. alpha = 1 - exp(-rate * dt) */
    {
        float dt_sec = (float)( cl.serverTime - hha_lastMagnetismTime ) * 0.001f;
        float rateYaw, ratePitch, alphaYaw, alphaPitch;

        if ( dt_sec <= 0.0f || dt_sec > 0.1f ) dt_sec = 0.0166f;
        hha_lastMagnetismTime = cl.serverTime;

        rateYaw   = -logf( 1.0f - pullYaw   ) / 0.0166f;
        ratePitch = -logf( 1.0f - pullPitch ) / 0.0166f;

        if ( pullYaw   <= 0.0f ) rateYaw   = 0.0f;
        if ( pullPitch <= 0.0f ) ratePitch = 0.0f;

        alphaYaw   = 1.0f - expf( -rateYaw   * dt_sec );
        alphaPitch = 1.0f - expf( -ratePitch * dt_sec );

        if ( alphaYaw   < 0.0f ) alphaYaw   = 0.0f;
        if ( alphaYaw   > 1.0f ) alphaYaw   = 1.0f;
        if ( alphaPitch < 0.0f ) alphaPitch = 0.0f;
        if ( alphaPitch > 1.0f ) alphaPitch = 1.0f;

        cl.viewangles[YAW]   += yawDiff   * alphaYaw;
        cl.viewangles[PITCH] += pitchDiff * alphaPitch;
    }
...
"""
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"[PATCHED] Aim assist v13.7 in {target_file}")
    return True
...
def patch_cl_main_namerotator():
    ...
    helper_code = r"""
/* ============================================================
 * [PATCHED v13.6] Name Color Rotator (File-Scope)
 * ============================================================ */
static int  hha_nameLastRotate = 0;
...
static void HHA_NameRotator(void) {
    ...
    char curName[128];
    char newName[128];
    int rotate, interval, scheme;

    rotate = Cvar_VariableIntegerValue("cg_handheldNameRotate");
    if (!rotate) return;

    interval = Cvar_VariableIntegerValue("cg_handheldNameInterval");
    if (interval < 100) interval = 100;
    if (cls.realtime - hha_nameLastRotate < interval) return;
    hha_nameLastRotate = cls.realtime;
    ...
    hha_nameCounter++;
    if (hha_nameCounter > 1000) hha_nameCounter = 0;
...
"""
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print("[PATCHED] Name rotator in cl_main.c (v13.6)")
    return True
...
def patch_cl_console_chatprefill():
    ...
    insert_code = (
        "\n\t/* [PATCHED v13.6] Automatischer Chat-Prefill */\n"
        "\t{\n"
...
def patch_cg_view():
    ...
    zoom_helper = r"""
/* ============================================================
 * [PATCHED v13.6] Dynamic Zoom + Auto-Switch (cgame-VM)
 * cgame-VM hat KEIN trap_Cvar_VariableValue!
 * ============================================================ */
...
def patch_cg_hitmarker():
    ...
    hitmarker_code = r"""
/* ============================================================
 * [PATCHED v13.6] Hitmarker + Damage-Indicator
 * ============================================================ */
...
"""
    with open(os.path.join(menu_dir, "settings_aimassist.menu"),
        ...
        f.write(aim_menu)
    print("[PATCHED] Menu: settings_aimassist.menu (v13.7 setcvar-fix)")
...
def main():
    ...
    applied = 0
    if is_smokinguns:
        print("==> Patching SmokinGuns (v13.7)")
        diagnostic_dump()
