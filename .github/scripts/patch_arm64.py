...
[EXTENDED v9] Handheld aim assist + Dynamic Zoom:
  ...
  - NEU: Dynamic Zoom (distanzbasierter Auto-Zoom in cg_view.c)
         Hook verifiziert gegen tatsaechliche Signatur:
             static int CG_CalcFov( void )
         FOV wird in cg.refdef.fov_x / cg.refdef.fov_y geschrieben.
         Distanz kommt per cvar cg_handheldAimAssistTargetDist (Client -> Cgame).
"""
...
# Cvar, die nur vom Client gesetzt wird (Client -> Cgame-Bruecke)
    ...
    if ui_local_path:
        ...
        if "ui_handheldAimAssistZoom" not in content:
            ...
            if anchor in content:
                decls = "".join(
                    f"extern vmCvar_t\tui_{c[3:]};\n" if c.startswith("cg_") else ""
                    for c, _ in AIM_CVARS
                )
                # build explicit list to avoid confusion
                decls = "".join(
...
/* Findet das beste Ziel. Setzt zusaetzlich cg_handheldAimAssistTargetDist
 * (Client -> Cgame Bridge), damit CG_HandheldApplyZoom() in cg_view.c
 * distanzbasierten Auto-Zoom anwenden kann. */
static int HHA_FindTarget( const hha_config_t *cfg, vec3_t bestDir, float *bestAngleOut ) {
    ...
    *bestAngleOut = bestAngle;
    ...
    /* [PATCHED v9] Distanz-Bruecke fuer Dynamic Zoom */
    if ( best >= 0 ) {
...
# ============================================================================
# DYNAMIC ZOOM (Cgame / cg_view.c)
# ============================================================================
def patch_cg_zoom():
    """Injiziert distanzbasierten Auto-Zoom in cg_view.c.

    Verifizierte Signatur aus Smokin' Guns Repo:
        static int CG_CalcFov( void )
    FOV wird direkt in cg.refdef.fov_x / cg.refdef.fov_y geschrieben
    (KEIN Pointer-Out wie in der alten Doku angenommen).
    Wir injizieren nach den Zuweisungen und modifizieren die Werte
    direkt; fov_y wird ueber die bestehende atan2-Formel neu berechnet.
    """
    target_file = None
    for root, _dirs, files in os.walk("code"):
    ...
    zoom_helper = r"""
/* ============================================================
 * [PATCHED v9] Dynamic Zoom - distanzbasierter Auto-Zoom
 * ============================================================
 * Liest cg_handheldAimAssistTargetDist (vom Client gesetzt,
 * via cvar-Bruecke) und modifiziert cg.refdef.fov_x / fov_y.
 * Wird aus CG_CalcFov() nach dem Setzen der FOV aufgerufen.
 * Wird NICHT angewendet, wenn:
 *   - cg.zoomed aktiv (echtes Scope)
 *   - Third-Person / Deathcam
 *   - Intermission / Spectator / tot
 * ============================================================ */
...
static void CG_HandheldApplyZoom( void ) {
    ...
    /* Kein Handheld-Zoom waehrend echtes Zielfernrohr aktiv ist */
    if ( cg.zoomed ) {
        return;
    }

    enabled = ( Cvar_VariableIntegerValue("cg_handheldAimAssistZoom") != 0 );
    ...
    dist      = Cvar_VariableValue("cg_handheldAimAssistTargetDist");
    nearDist  = Cvar_VariableValue("cg_handheldAimAssistZoomNear");
    farDist   = Cvar_VariableValue("cg_handheldAimAssistZoomFar");
    targetFov = Cvar_VariableValue("cg_handheldAimAssistZoomFov");
    speed     = Cvar_VariableValue("cg_handheldAimAssistZoomSpeed");

    /* Clamping gegen manuell editierte Configs */
    ...
    /* Ziel-Zoom-Level 0..1 basierend auf Distanz */
    ...
    /* Glaettung (exponentielles Lerp mit Delta-Zeit) */
    ...
    /* Bei Zielverlust schnell zurueck auf 0 */
    ...
    if ( cg_hhaZoomCurrent > 0.0005f ) {
        ...
        /* fov_y mit der bestehenden CG_CalcFov-Formel neu berechnen */
...
"""
    ...
    # 1) Helper vor CG_CalcFov-Comment-Block injizieren.
    #    Anker: das Kommentar "CG_CalcFov\n\nFixed fov at intermissions..."
    ...
    # 2) Hook nach der FOV-Zuweisung (verifizierte Stelle).
    #    Original:
    ...
    content = fov_set_pat.sub(
        r'\1\n\n\t/* [PATCHED v9] Dynamic Zoom (distanzbasiert) */\n'
        r'\tCG_HandheldApplyZoom();',
        content, count=1)

    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"[PATCHED] Dynamic Zoom in {target_file} (hook after fov_x/fov_y set)")
    return True
