...
v13.6 - Verifizierte Optimierungen + Bugfixes.
"""
...
def patch_aim_assist():
    ...
    helper_code = r"""
/* ============================================================
 * [PATCHED v13.6] Handheld Aim Assist (Client-Seite)
 * ============================================================ */
#include <math.h>
    ...
    for ( i = 0; i < cl.snap.numEntities && ncands < HHA_MAX_CANDIDATES; i++ ) {
        ...
        if ( ent->number == hha_lastTargetNum ) {
            float stickyDot = cosf( cfg->angle * 0.85f *
                                    ( HHA_PI / 180.0f ) );
            if ( dot > stickyDot ) {
                dot = ( dot + 1.0f ) * 0.5f;   /* künstlich verbessern */
            }
        }
...
static void CL_HandheldAimMagnetism( usercmd_t *cmd ) {
     ...
     * als Rate interpretiert werden. alpha = 1 - exp(-rate * dt) */
    {
        ...
        /* pullYaw = 0.55 bei 60 FPS → rate = -ln(1-0.55)/0.0166 ≈ 48.2 */
...
"""
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"[PATCHED] Aim assist v13.6 in {target_file}")
    return True
...
"""
    with open(os.path.join(menu_dir, "settings_aimassist.menu"),
        ...
        f.write(aim_menu)
    print("[PATCHED] Menu: settings_aimassist.menu (v13.6 setcvar-fix)")
...
def main():
    ...
    applied = 0
    if is_smokinguns:
        print("==> Patching SmokinGuns (v13.6)")
        diagnostic_dump()
