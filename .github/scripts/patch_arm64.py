...
[EXTENDED] Handheld aim assist v6:
  - Lead prediction (fixes bot-vs-online difference)
  - 5 new tunables (Pitch, Lead, Sticky, MaxDist, LOS)
  - Two-tier menu: Basic + Advanced
"""
...
def patch_client_cvar():
    ...
    with open(path, "r", encoding="utf-8", errors="ignore") as f:
        content = f.read()
    if 'cg_handheldAimAssistLead' in content:
    ...
    insertion = anchor + "\n" + (
        ...
        "\tCvar_Get (\"cg_handheldAimAssistPitch\", \"0.625\", CVAR_ARCHIVE);\n"
        "\tCvar_Get (\"cg_handheldAimAssistLead\", \"0\", CVAR_ARCHIVE);\n"
        "\tCvar_Get (\"cg_handheldAimAssistSticky\", \"10\", CVAR_ARCHIVE);\n"
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"[PATCHED] cl_main.c: 10 cvars registered")
    return True
...
def patch_ui_cvar():
    ...
    if ui_local_path:
        with open(ui_local_path, "r", encoding="utf-8", errors="ignore") as f:
            content = f.read()
        if "ui_handheldAimAssistLead" not in content:
            ...
            if anchor in content:
                ...
                with open(ui_local_path, "w", encoding="utf-8") as f:
                    f.write(content)
                print("[PATCHED] ui_local.h: 10 extern decls added")
    ...
    if "ui_handheldAimAssistLead;" not in content:
        ...
        if anchor in content:
            ...
            content = content.replace(anchor, decls + anchor, 1)
    if '"cg_handheldAimAssistLead"' not in content:
        ...
        if anchor in content:
            entries = (
                ...
                '\n\t{ &ui_handheldAimAssistPitch, "cg_handheldAimAssistPitch", "0.625", CVAR_ARCHIVE },'
                '\n\t{ &ui_handheldAimAssistLead, "cg_handheldAimAssistLead", "0", CVAR_ARCHIVE },'
                '\n\t{ &ui_handheldAimAssistSticky, "cg_handheldAimAssistSticky", "10", CVAR_ARCHIVE },'
    ...
    with open(ui_main_path, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"[PATCHED] ui_main.c: 10 cvars registered")
    return True
...
def patch_aim_assist():
    ...
    helper_code = r"""
/* ============================================================
 * [PATCHED] Handheld Aim Assist v6 mit Lead-Prediction
 * ============================================================
 *  10 CVARs:
 *    cg_handheldAimAssist            0/1     Master
 *    cg_handheldAimAssistFire        0/1     Fire-Assist
 *    cg_handheldAimAssistAngle       5..25   Kegel (Grad)
 *    cg_handheldAimAssistStrength    0.05..0.50  Magnetismus (Yaw)
 *    cg_handheldAimAssistFriction    0.50..1.00  Input-Daempfung
 *    cg_handheldAimAssistPitch       0.30..1.00  Pitch-Multiplier
 *    cg_handheldAimAssistLead        0..150      Lead in ms
 *    cg_handheldAimAssistSticky      0..30       Sticky-Frames
 *    cg_handheldAimAssistMaxDist     500..8000   Max Distanz
 *    cg_handheldAimAssistLOS         0/1         LOS-Check
 * ============================================================ */
#include <math.h>
...
static void HHA_ReadConfig( hha_config_t *cfg ) {
    ...
    if ( pitchMul >  1.00f )  pitchMul =  1.00f;
    if ( leadMs   <  0.0f )   leadMs   =  0.0f;
    if ( leadMs   > 150.0f )  leadMs   = 150.0f;
...
static void HHA_PredictPos( const entityState_t *ent, float leadTime, vec3_t out ) {
    out[0] = ent->pos.trBase[0] + ent->pos.trDelta[0] * leadTime;
    out[1] = ent->pos.trBase[1] + ent->pos.trDelta[1] * leadTime;
    out[2] = ent->pos.trBase[2] + ent->pos.trDelta[2] * leadTime;
}

static int HHA_FindTarget( const hha_config_t *cfg, vec3_t bestDir, float *bestAngleOut ) {
    int    i, best = -1;
    float  bestAngle = cfg->angle;
    vec3_t forward, toEnt, predicted, predFeet;
    ...
    *bestAngleOut = 999.0f;
    ...
    for ( i = 0; i < cl.snap.numEntities; i++ ) {
        ...
        HHA_PredictPos( ent, cfg->leadTime, predFeet );
        ...
        predicted[0] = dx; predicted[1] = dy; predicted[2] = dz;
        VectorNormalize( predicted );

        dot = DotProduct( forward, predicted );
        ...
        if ( angle < bestAngle ) {
            if ( cfg->losEnabled ) {
                if ( !HHA_HasLineOfSight( cl.snap.ps.origin, predFeet ) ) continue;
            }
            bestAngle = angle;
            best      = ent->number;
            VectorCopy( predicted, bestDir );
        }
    }
...
"""
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"[PATCHED] Aim assist v6 in {target_file}")
    return True
...
itemDef {
	name aim_hint1
	style 1
	text "Advanced options: Lead prediction for online play,"
	rect 60 210 520 16
	textalign ITEM_ALIGN_CENTER
	textalignx 260
	textaligny 12
...
itemDef {
	name aim_hint2
	style 1
	text "Pitch strength, Sticky, Max Distance, LOS."
	rect 60 227 520 16
	textalign ITEM_ALIGN_CENTER
	textalignx 260
	textaligny 12
...
"""
    ...
    # ---------------- settings_aimassist_advanced.menu (NEW) ----------------
...
itemDef {
	name aim_subtitle
	style 1
	text "Fine tuning. For online play, Lead = ping/2 is a good start."
	rect 60 32 520 18
	textalign ITEM_ALIGN_CENTER
	textalignx 260
	textaligny 14
...
itemDef {
	...
	cvarfloat "cg_handheldAimAssistPitch" 0.625 0.30 1.00
	rect 80 65 220 20
	textalign ITEM_ALIGN_RIGHT
	textalignx 140
	textaligny 12
...
itemDef {
	...
	group grpAim
	type ITEM_TYPE_SLIDER
	text "Lead Prediction (ms):"
	cvarfloat "cg_handheldAimAssistLead" 0 0 150
	rect 80 90 220 20
	textalign ITEM_ALIGN_RIGHT
	textalignx 140
	textaligny 12
...
itemDef {
	...
	cvarfloat "cg_handheldAimAssistSticky" 10 0 30
	rect 80 115 220 20
	textalign ITEM_ALIGN_RIGHT
	textalignx 140
	textaligny 12
...
itemDef {
	name aim_hint1
	style 1
	text "Pitch: vertical pull strength (0.625 = default ratio)"
	rect 60 200 520 16
	textalign ITEM_ALIGN_CENTER
	textalignx 260
	textaligny 12
...
itemDef {
	name aim_hint2
	style 1
	text "Lead: aim ahead by ms (30-80 typical for online, 0 for bots)"
	rect 60 217 520 16
	textalign ITEM_ALIGN_CENTER
	textalignx 260
	textaligny 12
...
itemDef {
	name aim_hint3
	style 1
	text "Sticky: how long target stays locked. LOS: don't aim through walls."
	rect 60 234 520 16
	textalign ITEM_ALIGN_CENTER
	textalignx 260
	textaligny 12
...
"""
    ...
    with open(os.path.join(menu_dir, "settings_aimassist_advanced.menu"),
              "w", encoding="utf-8") as f:
        f.write(aim_adv)
    print("[PATCHED] Menu: settings_aimassist_advanced.menu (new)")
