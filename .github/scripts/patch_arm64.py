...
PRESETS = {
    "online": {
        ...
        "cg_handheldAimAssistZoomNear": "80",
        "cg_handheldAimAssistZoomFar": "1200",
        "cg_handheldAimAssistZoomFov": "30",
        "cg_handheldAimAssistZoomSpeed": "25",
        "_echo": "^1[Preset] ^7ONLINE - SNAP-ON (Unlagged-optimiert) geladen",
    },
    "offline": {
        ...
        "_echo": "^2[Preset] ^7OFFLINE (Bots, sanft) geladen",
    },
    "aimbot": {
        ...
        "cg_handheldAimAssistZoomFov": "30",
        "cg_handheldAimAssistZoomSpeed": "25",
        "_echo": "^1[Preset] ^7AIMBOT MAX - SNAP-ON (Unlagged-optimiert) geladen",
    },
}
...
def patch_cg_hitmarker():
    """
    v14.2: Kompakter, kontrastreicher Hitmarker.
    4 Quadrate in X-Form, hellgelb (Kill: rot), 180ms fade.
    """
    target_file = None
    for root, _dirs, files in os.walk("code"):
    ...
    if "HHA_HITMARKER_V2" in content:
        print("[SKIP] Hitmarker v14.2 bereits gepatcht")
        return True
...
/* ============================================================
 * [PATCHED v14.2] Hitmarker - klein, X-foermig, kontrastreich
 * HHA_HITMARKER_V2
 * ============================================================ */
...
'''
    ...
    if draw2d_pat.search(content):
        content = draw2d_pat.sub(
            r'\1\n\n\t/* HHA_HITMARKER_V2 */\n\tCG_HandheldDrawOverlays();',
            content, count=1)
        print("[PATCHED] Hitmarker v14.2 Draw-Call")

    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print("[PATCHED] Hitmarker v14.2 in cg_draw.c")
    return True
...
def patch_vm_force_native():
    ...
    for root, _dirs, files in os.walk("code"):
        if "vm.c" in files and "qcommon" in root:
            vm_c = os.path.join(root, "qcommon", "vm.c")
            break
    if not vm_c:
...
itemDef { name p_1 group grpAim type ITEM_TYPE_BUTTON text "1 - ONLINE (Snap-On, unlagged)" rect 100 68 440 20 textalign ITEM_ALIGN_CENTER textalignx 220 textaligny 14 textscale .26 forecolor 1 .75 0 1 visible 1 action { play "sound/misc/menu3.wav" ; setcvar "cg_handheldAimPreset" "1" ; exec "preset_online.cfg" ; close aim_assist_menu ; wait 50 ; open aim_assist_menu } }
itemDef { name p_2 group grpAim type ITEM_TYPE_BUTTON text "2 - OFFLINE (Bots, sanft)" rect 100 90 440 20 textalign ITEM_ALIGN_CENTER textalignx 220 textaligny 14 textscale .26 forecolor .3 1 .3 1 visible 1 action { play "sound/misc/menu3.wav" ; setcvar "cg_handheldAimPreset" "2" ; exec "preset_offline.cfg" ; close aim_assist_menu ; wait 50 ; open aim_assist_menu } }
itemDef { name p_3 group grpAim type ITEM_TYPE_BUTTON text "3 - AIMBOT (MAX Snap-On)" rect 100 112 440 20 textalign ITEM_ALIGN_CENTER textalignx 220 textaligny 14 textscale .26 forecolor 1 .3 .3 1 visible 1 action { play "sound/misc/menu3.wav" ; setcvar "cg_handheldAimPreset" "3" ; exec "preset_aimbot.cfg" ; close aim_assist_menu ; wait 50 ; open aim_assist_menu } }
itemDef { name p_0 group grpAim type ITEM_TYPE_BUTTON text "0 - CUSTOM (handheld.cfg)" rect 100 134 440 20 textalign ITEM_ALIGN_CENTER textalignx 220 textaligny 14 textscale .26 forecolor .85 .85 .85 1 visible 1 action { play "sound/misc/menu3.wav" ; setcvar "cg_handheldAimPreset" "0" ; exec "handheld.cfg" ; close aim_assist_menu ; wait 50 ; open aim_assist_menu } }
    ...
    for name, data in PRESETS.items():
        ...
        for d in (menu_dir, preset_dir):
            with open(os.path.join(d, f"preset_{name}.cfg"),
                      "w", encoding="utf-8") as f:
                f.write(body)
    print("[PATCHED] Presets: preset_online/offline/aimbot.cfg")
...
def main():
    ...
    applied = 0
    if is_smokinguns:
        print("==> Patching SmokinGuns (v14.2)")
        diagnostic_dump()
