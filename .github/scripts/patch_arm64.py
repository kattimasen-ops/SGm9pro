...
v13.4 - Fixes:
  - Fix: showCvar/hideCvar Syntax in settings_aimassist.menu
    (verhinderte Ueberlagerung der Preset-Buttons)
  - Fix: Preset-Konsistenz-Layer - Sub-Cvars werden bei Preset-Wechsel
    garantiert angewendet, sonst wechselt Preset auf Custom
  - Neu: aim_dump / aim_sync Console-Commands
  - Fix: Preset-CFG-Dateien liegen im Base-Game-Dir (nicht ui/),
    damit 'exec preset_aimbot.cfg' funktioniert

Alle vorherigen Features:
  - v13.3: Name Color Rotator (cg_handheldNameRotate/Interval/Scheme)
  - v13.3: Chat-Prefill (cg_handheldChatPrefix) fuer T und Y
  - v13.3.1: Name-Rotator Helper auf File-Scope (kein invalid-storage-Fehler)
  - v13.2: Config-Cache, LOS-Budget 1, TargetDist nur bei Aenderung
  - v13.2: cg_handheldAimAssistHardLock (instant Snap AimBot)
  - v13.2: ingame_options.menu wird ins Release-Verzeichnis kopiert
  - v13.1: Respawn-Reset mit 4 unabhaengigen Triggern
"""
...
def patch_client_cvar():
    ...
    lines = [
        "\t/* [PATCHED v13.4] Handheld Aim Assist + Name Rotator + Chat Prefill */",
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write(content)
    print("[PATCHED] cl_main.c: 28 cvars registered")
    return True
...
def patch_aim_assist():
    ...
    helper_code = r"""
/* ============================================================
 * [PATCHED v13.4] Handheld Aim Assist + Preset Consistency
 * ============================================================ */
#include <math.h>
...
/* ------------------------------------------------------------
 * [v13.4] Preset Consistency Layer
 * ------------------------------------------------------------ */
...
static void HHA_Cmd_AimDump( void ) {
    Com_Printf( "===== Handheld Aim Assist - Aktuelle Werte =====\n" );
    ...
    Com_Printf( "  Fire          : %d\n", Cvar_VariableIntegerValue("cg_handheldAimAssistFire") );
    ...
    Com_Printf( "  Friction      : %.2f\n", Cvar_VariableValue("cg_handheldAimAssistFriction") );
    Com_Printf( "  PitchMul      : %.2f\n", Cvar_VariableValue("cg_handheldAimAssistPitch") );
    Com_Printf( "  Sticky        : %d\n", Cvar_VariableIntegerValue("cg_handheldAimAssistSticky") );
    Com_Printf( "  SnapAngle     : %.2f\n", Cvar_VariableValue("cg_handheldAimAssistSnapAngle") );
    Com_Printf( "  MaxDist       : %.2f\n", Cvar_VariableValue("cg_handheldAimAssistMaxDist") );
    Com_Printf( "  LOS           : %d\n", Cvar_VariableIntegerValue("cg_handheldAimAssistLOS") );
    Com_Printf( "  UnlaggedSync  : %d\n", Cvar_VariableIntegerValue("cg_handheldAimAssistUnlaggedSync") );
    Com_Printf( "  Interpolate   : %d\n", Cvar_VariableIntegerValue("cg_handheldAimAssistInterpolate") );
    Com_Printf( "  Zoom          : %d\n", Cvar_VariableIntegerValue("cg_handheldAimAssistZoom") );
    Com_Printf( "  AutoSwitch    : %d\n", Cvar_VariableIntegerValue("cg_handheldAutoSwitch") );
    Com_Printf( "  HitMarker     : %d\n", Cvar_VariableIntegerValue("cg_handheldHitMarker") );
    Com_Printf( "  DamageInd     : %d\n", Cvar_VariableIntegerValue("cg_handheldDamageIndicator") );
...
static void HHA_Cmd_AimSync( void ) {
    ...
    if ( p == 0 ) {
        Com_Printf( "Aim-Assist: Custom-Preset aktiv - nichts zu synchronisieren.\n" );
        return;
    }
...
"""
    ...
    if not createcmd_pat.search(content):
        ...
        content = createcmd_pat.sub(
            ...
            content, count=1)
        print("[PATCHED] Preset-Consistency + Cache + Respawn in CL_CreateCmd")
    ...
    if cmdreg_anchor in content:
        ...
        print("[WARN] centerview anchor not found (aim_dump not registered)")

    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"[PATCHED] Aim assist v13.4 in {target_file}")
    return True
...
def patch_cl_main_namerotator():
    ...
    helper_code = r"""
/* ============================================================
 * [PATCHED v13.4] Name Color Rotator
 * Helper auf FILE-SCOPE (vor CL_Frame), Aufruf in CL_Frame.
 * ============================================================ */
...
"""
    ...
    if call_anchor not in content:
        print("[WARN] CL_CheckUserinfo anchor not found (call insert)")
        return False
    content = content.replace(call_anchor,
                              call_anchor + "\n\n\tHHA_NameRotator();",
                              1)

    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print("[PATCHED] Name rotator in cl_main.c (v13.4, file-scope)")
    return True
...
def patch_cl_console_chatprefill():
    ...
    insert_code = (
        "\n\t/* [PATCHED v13.3] Automatischer Chat-Prefill */\n"
        "\t{\n"
...
def patch_cg_view():
    ...
    zoom_helper = r"""
/* ============================================================
 * [PATCHED v13.4] Dynamic Zoom + Auto-Switch
 * ============================================================ */
...
def patch_cg_hitmarker():
    ...
    hitmarker_code = r"""
/* ============================================================
 * [PATCHED v13.4] Hitmarker + Damage-Indicator
 * ============================================================ */
...
itemDef { name p_online_inactive group grpAim type ITEM_TYPE_BUTTON text "Preset: ONLINE (Unlagged)" rect 100 202 200 20 textalign ITEM_ALIGN_CENTER textalignx 100 textaligny 14 textscale .26 forecolor 1 .75 0 1 visible 1 cvarTest "cg_handheldAimPreset" hideCvar { "1" } action { play "sound/misc/menu3.wav" ; exec "preset_online.cfg" } }
itemDef { name p_online_active   group grpAim type ITEM_TYPE_BUTTON text "* ONLINE (Unlagged) *"    rect 100 202 200 20 textalign ITEM_ALIGN_CENTER textalignx 100 textaligny 14 textscale .26 forecolor .3 1 .3 1 visible 1 cvarTest "cg_handheldAimPreset" showCvar { "1" } action { play "sound/misc/menu3.wav" ; exec "preset_online.cfg" } }

itemDef { name p_offline_inactive group grpAim type ITEM_TYPE_BUTTON text "Preset: OFFLINE (Bots)" rect 100 226 200 20 textalign ITEM_ALIGN_CENTER textalignx 100 textaligny 14 textscale .26 forecolor 1 .75 0 1 visible 1 cvarTest "cg_handheldAimPreset" hideCvar { "2" } action { play "sound/misc/menu3.wav" ; exec "preset_offline.cfg" } }
itemDef { name p_offline_active   group grpAim type ITEM_TYPE_BUTTON text "* OFFLINE (Bots) *"     rect 100 226 200 20 textalign ITEM_ALIGN_CENTER textalignx 100 textaligny 14 textscale .26 forecolor .3 1 .3 1 visible 1 cvarTest "cg_handheldAimPreset" showCvar { "2" } action { play "sound/misc/menu3.wav" ; exec "preset_offline.cfg" } }

itemDef { name p_aimbot_inactive group grpAim type ITEM_TYPE_BUTTON text "Preset: AIMBOT (MAX)"      rect 100 250 200 20 textalign ITEM_ALIGN_CENTER textalignx 100 textaligny 14 textscale .26 forecolor 1 .3 .3 1 visible 1 cvarTest "cg_handheldAimPreset" hideCvar { "3" } action { play "sound/misc/menu3.wav" ; exec "preset_aimbot.cfg" } }
itemDef { name p_aimbot_active   group grpAim type ITEM_TYPE_BUTTON text "*** AIMBOT (MAX) ***"      rect 100 250 200 20 textalign ITEM_ALIGN_CENTER textalignx 100 textaligny 14 textscale .28 forecolor 1 .1 .1 1 visible 1 cvarTest "cg_handheldAimPreset" showCvar { "3" } action { play "sound/misc/menu3.wav" ; exec "preset_aimbot.cfg" } }
...
"""
    with open(os.path.join(menu_dir, "settings_aimassist.menu"),
        ...
        f.write(aim_menu)
    print("[PATCHED] Menu: settings_aimassist.menu (v13.4 showCvar-fix)")
...
"""
    ...
    # ---- Presets in BASE game dir (fuer 'exec' erreichbar) ----
...
"""
    for d in (menu_dir, preset_dir):
        with open(os.path.join(d, "preset_online.cfg"),
                  "w", encoding="utf-8") as f:
...
"""
    for d in (menu_dir, preset_dir):
        with open(os.path.join(d, "preset_offline.cfg"),
                  "w", encoding="utf-8") as f:
...
"""
    for d in (menu_dir, preset_dir):
        with open(os.path.join(d, "preset_aimbot.cfg"),
                  "w", encoding="utf-8") as f:
...
def main():
    ...
    applied = 0
    if is_smokinguns:
        print("==> Patching SmokinGuns (v13.4)")
        diagnostic_dump()
