...
v11.3 - Finale Version:
  - settings_options.menu wird IN-PLACE gepatcht (Boost FPS -> Button danach)
  - QoL-Toggles (AutoSwitch/HitMarker/DamageIndicator) im Aim-Assist-Untermenue
  - CM_PointLeafnum/CM_LeafArea entfernt (in SG nicht verfuegbar)
  - Alle anderen Funktionen aus v11.2 unveraendert
"""
...
def patch_aim_assist():
    ...
    helper_code = r"""
/* ============================================================
 * [PATCHED v11.3] Handheld Aim Assist (Input-Kurve + Magnetismus)
...
"""
    ...
    with open(target_file, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"[PATCHED] Aim assist v11.3 in {target_file}")
    return True
...
def patch_cg_view():
    ...
    zoom_helper = r"""
/* ============================================================
 * [PATCHED v11.3] Dynamic Zoom + Auto-Weapon-Switcher (cg_view.c)
 * ============================================================ */
...
/* ============================================================
 * [PATCHED v11.3] Auto-Weapon-Switcher
...
def patch_cg_hitmarker():
    ...
    hitmarker_code = r"""
/* ============================================================
 * [PATCHED v11.3] Hitmarker + Damage-Richtungsanzeige
...
"""
    ...
    if score_anchor.search(content):
        content = score_anchor.sub(
            r'\n\n\t/* [PATCHED v11.3] Hitmarker + Damage-Indicator */\n'
            r'\tCG_DrawHitMarker();\n'
...
def patch_cg_event_hit():
    ...
    decls = ("\n/* [PATCHED v11.3] Externer Hitmarker-Helper */\n"
             "void CG_RegisterHitMarker( void );\n\n")
    ...
    if bullet_case.search(content):
        content = bullet_case.sub(
            r'\1\n\t\t/* [PATCHED v11.3] Hitmarker nur wenn wir der Shooter sind */\n'
...
# ============================================================================
# settings_options.menu IN-PLACE PATCH
# ============================================================================
def patch_settings_options_menu(menu_dir):
    """Fuegt einen 'Handheld Aim Assist...' Button nach dem Boost-FPS-Item ein.
    Die originale Struktur bleibt unveraendert."""
    path = os.path.join(menu_dir, "settings_options.menu")
    if not os.path.exists(path):
        print("[WARN] settings_options.menu not found - Button not added")
        return False
    with open(path, "r", encoding="utf-8", errors="ignore") as f:
        content = f.read()
    if "aim_assist_menu" in content:
        print("[SKIP] settings_options.menu already patched")
        return True
    if 'cvar "cg_boostfps"' not in content:
        print("[WARN] cg_boostfps anchor not found in settings_options.menu")
        return False

    # Match: 'cvar "cg_boostfps"' bis zum ersten '}' danach (Ende des itemDef)
    pattern = re.compile(r'(cvar\s+"cg_boostfps"[^}]*\})')
    insertion = """
	itemDef {
      		name options
		group grpOptions
...
"""
    new_content, n = pattern.subn(r'\1' + insertion, content, count=1)
    if n == 0:
        print("[WARN] settings_options.menu: insertion failed")
        return False
    with open(path, "w", encoding="utf-8") as f:
        f.write(new_content)
    print("[PATCHED] settings_options.menu: 'Handheld Aim Assist...' button added")
    return True
...
def write_menu_override():
    ...
    # ---------- settings_options.menu in-place patchen ----------
    patch_settings_options_menu(menu_dir)
...
def main():
    ...
    applied = 0
    if is_smokinguns:
        print("==> Patching SmokinGuns (v11.3)")
        diagnostic_dump()
