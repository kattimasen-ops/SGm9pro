...
v15.5-fix - main() Reihenfolge korrigiert: Typen-definierende
            Patches werden ZULETZT aufgerufen, damit sie oben
            in cg_draw.c landen.
"""
...
def patch_cg_hha_radar():
    """v15.3: DEFINIERT hha_threat_t und Helfer. MUSS zuletzt aufgerufen werden."""
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_draw.c" in files:
            target_file = os.path.join(root, "cg_draw.c")
...
static int          hha_threatLosBudget = 0;
...
void CG_HHA_Radar_Update( void ) {
    ...
    hha_threatNum = 0;
    hha_threatLosBudget = HHA_THREAT_LOS_BUDGET;
...
'''
    ...
    if "CG_HHA_Radar_Update();" not in content:
        hook_pat = re.compile(
...
def patch_hud_compass():
    """v15.2: BENUTZT hha_threat_t. Muss VOR patch_cg_hha_radar aufgerufen werden."""
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_draw.c" in files:
            target_file = os.path.join(root, "cg_draw.c")
    ...
    if "HHA_COMPASS_V1" in content:
        ...
        content = content.replace("CG_HandheldDrawCompass();\n\t", "")
 ...
 * HHA_COMPASS_V2
 * Benoetigt: hha_threat_t, hha_threatCache, hha_threatNum
 *            (definiert in HHA_RADAR_V3)
 * ============================================================ */
#include <math.h>
...
'''
    ...
    if hook_pat.search(content):
        ...
        print("[WARN] Compass: CG_HHA_Radar_Update anchor fehlt")
...
def patch_hud_mini_radar():
    """v15.5: BENUTZT hha_threat_t. Muss VOR patch_cg_hha_radar aufgerufen werden."""
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_draw.c" in files:
            target_file = os.path.join(root, "cg_draw.c")
    ...
    if "HHA_MINIRADAR_V1" in content:
        ...
        content = content.replace("CG_HHA_DrawMiniRadar();\n\t", "")
 ...
 * HHA_MINIRADAR_V2
 * Benoetigt: hha_threat_t, hha_threatCache, hha_threatNum,
 *            HHA_Threat_CvarFloat (definiert in HHA_RADAR_V3)
 * ============================================================ */
#include <math.h>
...
void CG_HHA_DrawMiniRadar( void ) {
    ...
    for ( i = 0; i < hha_threatNum; i++ ) {
        ...
        if ( offScreen ) {
            float ax, ay;
            ax = cx + sinf( angleRad ) * ( r_out + 5.0f );
            ay = cy - cosf( angleRad ) * ( r_out + 5.0f );
...
def main():
    ...
    applied = 0
    if is_smokinguns:
        print("==> Patching SmokinGuns (v15.5-fix)")
        diagnostic_dump()
        ...
        # --- cg_draw.c Patches ---
        # WICHTIG: Reihenfolge!
        # Jeder Patch fuegt seinen Code VOR den vorherigen ein.
        # => Der ZULETZT aufgerufene Patch landet ganz OBEN in der Datei.
        # => Patches, die Typen BENUTZEN, muessen ZUERST aufgerufen werden.
        # => Patches, die Typen DEFINIEREN, muessen ZULETZT aufgerufen werden.
        ...
        # Zuletzt: definiert hha_threat_t und Helper => landet ganz oben
        patch_cg_hha_radar()
