...
v14.6 - EnemyOutline-Cvars in AIM_CVAR_SUFFIXES ergaenzt.
v14.7 - Aim-Assist: EF_NODRAW-Filter, Map-Bounds-Pruefung,
        MASK_PLAYERSOLID fuer LoS.
v14.8 - Manuelles Entsperren per Joystick, Auto-Unlock bei
        Sichtverlust, Lock-Timeout.
v14.9 - Crosshair Mindestgroesse 25, Damage-Indicator groesser,
        Rename-Centerprint-Filter (cg_servercmds.c).
"""
...
def patch_aim_assist():
    """
    v14.8: Target-Scan laeuft NACH Maus/Joystick-Input.
    Sichtlinie: MASK_PLAYERSOLID (inkl. CONTENTS_PLAYERCLIP).
    EF_NODRAW-Filter + Map-Bounds-Pruefung.
    Manuelles Entsperren per Joystick.
    Auto-Unlock bei Sichtverlust (Lock-Timeout).
    """
    target_file = None
    for root, _dirs, files in os.walk("code"):
...
static qboolean HHA_CheckManualUnlock( void ) {
    ...
    if ( mag >= HHA_UNLOCK_MOVE_THRESHOLD ) {
        HHA_Log("Manual unlock: joystick move mag=%.1f (threshold=%.1f)",
                mag, HHA_UNLOCK_MOVE_THRESHOLD);
        return qtrue;
    }
...
static void CL_HandheldAimMagnetism( usercmd_t *cmd ) {
    ...
    if ( cfg->hardLock && hha_lockLastVisibleTime > 0 ) {
        ...
        if ( timeSinceVisible > HHA_LOCK_TIMEOUT_MS ) {
            HHA_Log("Lock timeout: no LoS for %d ms, releasing", timeSinceVisible);
            hha_cachedTarget = -1;
            hha_cachedValid  = qfalse;
            hha_lastTargetNum = -1;
            hha_lockAcquiredTime = 0;
...
def patch_cg_crosshair_min_size():
    """
    v14.9: Erzwingt eine Mindest-Crosshair-Groesse von 25.
    Die Engine liest cg_crosshairSize aus, das frueher auf 24 stand.
    Einige Builds haben es auf 16 geaendert, was auf dem R36S-Display
    zu klein ist. Wir setzen eine harte Untergrenze.
    """
    target_file = None
    for root, _dirs, files in os.walk("code"):
    ...
    # Match the line inside CG_DrawCrosshair (and CG_DrawCrosshair3D):
    #   w = h = cg_crosshairSize.value;
    # We insert a minimum clamp right after.
    ...
    # Also patch the 3D variant (only sets w, not h)
...
def patch_cg_hitmarker():
    """v14.9: Hitmarker X-Form + groesserer Damage-Indicator (Ring-Dots)."""
    target_file = None
    for root, _dirs, files in os.walk("code"):
        if "cg_draw.c" in files:
            target_file = os.path.join(root, "cg_draw.c")
    ...
    if "HHA_HITMARKER_V4" in content:
        print("[SKIP] Hitmarker/Damage-Indicator v14.9 bereits gepatcht")
        return True
 ...
 * HHA_HITMARKER_V4
 * - Hitmarker: kleines X, gelb (normal) / rot (Kill)
 * - Damage-Indicator: groesserer Ring mit dickeren Punkten
 * ============================================================ */
#include <math.h>
...
void CG_DrawDamageIndicator( void ) {
    ...
    cx = 320.0f;
    cy = 240.0f;
    /* v14.9: Radius vergroessert 90 -> 120 fuer bessere Sichtbarkeit */
    radius = 120.0f;
    ...
    for ( i = -2; i <= 2; i++ ) {
        ...
        /* v14.9: Punktgroesse vergroessert 10 -> 14, Minimum 3 -> 5 */
    ...
    if ( alpha > 0.3f ) {
        /* v14.9: Mittelpunkt groesser 4 -> 6 */
...
def patch_cg_servercmds_rename_filter():
    """
    v14.9: Filtert Center-Print-Nachrichten, die nur den eigenen
    Spielernamen enthalten. Der Smokin' Guns Server sendet bei einer
    Namensaenderung ein 'cp <Name>' an alle Clients, was den neuen
    Namen gross in der Bildschirmmitte anzeigt. Auf einem Handheld
    ohne Tastatur ist das unerwuenscht.
    """
    ...
    if not os.path.exists(target_file):
        for root, _dirs, files in os.walk("code"):
            if "cg_servercmds.c" in files:
                ...
                break
    if not target_file or not os.path.exists(target_file):
        print("[WARN] cg_servercmds.c not found (rename filter)")
        return False
    with open(target_file, "r", encoding="utf-8", errors="ignore") as f:
        content = f.read()
    ...
    new_block = (
        ...
        '\t\t * Hintergrund: Der SG-Server sendet bei Namensaenderung\n'
        '\t\t * ein "cp <Name>", was den Namen gross in der Mitte zeigt.\n'
        '\t\t */\n'
        '\t\t{\n'
        ...
        '\t\t\t\tstrippedName[sj] = 0;\n'
        '\n'
        '\t\t\t\t/* Wenn der Center-Print nur unser Name ist, unterdruecken */\n'
...
def write_aim_menu(menu_dir):
    """
    v14.8: Preset-Auswahl als ITEM_TYPE_MULTI (Dropdown).
    """
...
def main():
    ...
    applied = 0
    if is_smokinguns:
        print("==> Patching SmokinGuns (v14.9)")
        diagnostic_dump()
